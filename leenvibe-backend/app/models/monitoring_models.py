...
class ImpactAssessment(BaseModel):
    ...
    # Impact metrics
    coupling_impact_score: float = Field(
        default=0.0, description="Impact on coupling (0-1)"
    )
    complexity_impact_score: float = Field(
        default=0.0, description="Impact on complexity (0-1)"
    )
    test_coverage_impact: float = Field(
        default=0.0, description="Impact on test coverage (0-1)"
    )
...
class MonitoringConfiguration(BaseModel):
    ...
    debounce_delay_ms: int = Field(
        default=500, description="Delay before processing changes"
    )
    batch_size: int = Field(
        default=10, description="Maximum changes to process in one batch"
    )
    ...
    max_file_size_mb: int = Field(
        default=10, description="Maximum file size to analyze"
    )
    analysis_timeout_seconds: int = Field(
        default=30, description="Timeout for analysis operations"
    )
...
class MonitoringSession(BaseModel):
    ...
    configuration: MonitoringConfiguration = Field(
    ...
    recent_changes: List[FileChange] = Field(
        default_factory=list, description="Recent changes (last 100)"
    )
    pending_analysis: List[str] = Field(
        default_factory=list, description="Files pending analysis"
    )
