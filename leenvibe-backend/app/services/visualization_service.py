...
from ..models.visualization_models import (
    ...
    DiagramData, DiagramNode, DiagramEdge, MermaidDiagram,
    DiagramGenerationRequest, DiagramGenerationResponse,
    DiagramCache, VisualizationMetrics, VisualizationError,
    InteractiveElement, NodeStyle, EdgeStyle
)
from ..models.graph_models import GraphVisualizationData, ArchitecturePattern
...
"""
    ...
    async def generate_diagram(
        ...
        try:
            logger.info(f"Generating {request.configuration.diagram_type} diagram for project {request.project_id}")
            ...
            mermaid_content = await self._generate_mermaid_content(diagram_data, request.configuration)
            ...
            mermaid_diagram = MermaidDiagram(
                ...
                theme=request.configuration.theme,
                configuration=self.theme_configurations.get(request.configuration.theme, {})
            )
            
            # Cache if requested
            if request.cache_result:
                await self._cache_diagram(cache_key, diagram_data, mermaid_content, request.configuration)
            ...
            logger.info(f"Generated diagram in {generation_time}ms: {diagram_data.get_node_count()} nodes, {diagram_data.get_edge_count()} edges")
            return response
            
        except Exception as e:
            logger.error(f"Error generating diagram: {e}")
            raise VisualizationError(f"Failed to generate diagram: {str(e)}", "GENERATION_ERROR")
    
    async def _create_diagram_data(self, request: DiagramGenerationRequest) -> DiagramData:
    ...
    async def _create_architecture_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
        ...
        try:
            ...
            viz_data = await graph_service.get_visualization_data(project_id, max_nodes=config.max_nodes)
            ...
            for edge_data in viz_data.edges[:config.max_edges_per_diagram]:
                if edge_data["source"] in [n.metadata["original_id"] for n in nodes] and \
                   edge_data["target"] in [n.metadata["original_id"] for n in nodes]:
                    edge = DiagramEdge(
            ...
            return DiagramData(
                ...
                title="Architecture Overview",
                configuration=config,
                nodes=[DiagramNode(id="error", label="Error loading architecture", type="error")],
                edges=[]
            )
    
    async def _create_dependency_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
        ...
        try:
            ...
            for node_data in viz_data.nodes:
                node_type = "external" if "external" in node_data.get("label", "").lower() else "file"
                node = DiagramNode(
                    id=self._sanitize_id(node_data["id"]),
                    label=Path(node_data["name"]).stem if node_type == "file" else node_data["name"],
                    type=node_type,
                    properties=node_data,
    ...
    async def _create_hotspot_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
    ...
    async def _create_circular_deps_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
    ...
    async def _create_coupling_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
        ...
        try:
            ...
            for file_info in high_coupling_files[:config.max_nodes]:
                coupling_level = "high" if file_info["total_coupling"] > 10 else "medium" if file_info["total_coupling"] > 5 else "low"
    ...
    async def _create_class_hierarchy_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
        ...
        return DiagramData(
            ...
            configuration=config,
            nodes=[DiagramNode(id="placeholder", label="Class hierarchy analysis coming soon", type="placeholder")],
            edges=[]
        )
    
    async def _create_component_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
        ...
        return DiagramData(
            ...
            configuration=config,
            nodes=[DiagramNode(id="placeholder", label="Component analysis coming soon", type="placeholder")],
            edges=[]
        )
    
    async def _create_call_flow_diagram(self, project_id: str, config: DiagramConfiguration) -> DiagramData:
        ...
        return DiagramData(
            ...
            configuration=config,
            nodes=[DiagramNode(id="placeholder", label="Call flow analysis coming soon", type="placeholder")],
            edges=[]
        )
    
    async def _generate_mermaid_content(self, diagram_data: DiagramData, config: DiagramConfiguration) -> str:
        """Generate Mermaid.js content from diagram data"""
        try:
            template_str = self.templates.get(config.diagram_type)
            if not template_str:
                raise VisualizationError(f"No template found for diagram type: {config.diagram_type}")
            ...
            if config.diagram_type == DiagramType.CLASS_HIERARCHY:
                ...
                template_vars.update({
                    "participants": diagram_data.nodes,
                    "calls": diagram_data.edges
                })
    ...
    def _prepare_edges_for_template(self, edges: List[DiagramEdge], config: DiagramConfiguration) -> List[Dict[str, Any]]:
    ...
    def _generate_style_classes(self, diagram_data: DiagramData, config: DiagramConfiguration) -> Dict[str, str]:
        """Generate CSS style classes for diagram elements"""
        styles = {}
        
        # Theme-based base styles
        if config.theme == DiagramTheme.DARK:
            styles["default"] = "fill:#424242,stroke:#4fc3f7,stroke-width:2px,color:#ffffff"
        elif config.theme == DiagramTheme.LIGHT:
            styles["default"] = "fill:#f5f5f5,stroke:#0066cc,stroke-width:2px,color:#333333"
    ...
    def _group_nodes_as_components(self, nodes: List[DiagramNode]) -> List[Dict[str, Any]]:
    ...
    def _sanitize_id(self, id_str: str) -> str:
        ...
        sanitized = id_str.replace("/", "_").replace(".", "_").replace("-", "_").replace(" ", "_")
    ...
    async def _cleanup_cache(self):
        ...
        try:
            ...
            if len(self.cache) >= self.max_cache_size:
                ...
                for key, _ in sorted_entries[:len(self.cache) - self.max_cache_size + 10]:
                    del self.cache[key]
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_hit_rate": len([m for m in self.metrics if m.cache_hit]) / max(len(self.metrics), 1),
            "average_generation_time": sum(m.generation_time_ms for m in self.metrics) / max(len(self.metrics), 1)
        }
