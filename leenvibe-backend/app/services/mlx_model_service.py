...
class MLXModelService:
        ...
        try:
            ...
            if PRODUCTION_MODEL_AVAILABLE:
                ...
                if success:
                    ...
                    logger.info(f"Production service initialized in {deployment_mode} mode")
    ...
    def _check_mlx_availability(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"MLX Metal backend available. Active memory: {memory_info / 1024 / 1024:.1f}MB")
                return True
            except Exception as e:
    ...
    async def generate_text(self, prompt: str, max_tokens: Optional[int] = None) -> str:
        ...
        try:
            ...
            if self.production_service:
                ...
                response = await self.production_service.generate_text(prompt, max_tokens)
            ...
            # Fallback to simple model if available
                ...
                response = await self.simple_model_service.generate_text(prompt, max_tokens)
                ...
                logger.info(f"Simple model generation completed in {generation_time:.2f}s")
                return response
            else:
    ...
    async def _generate_mock_response(self, prompt: str) -> str:
        ...
        if "code" in prompt_lower and any(lang in prompt_lower for lang in ["python", "swift", "javascript"]):
