...
class SymbolDependencyTracker:
    ...
    def __init__(self):
        ...
        self.active_watchers: Dict[str, Set[str]] = defaultdict(set)  # file_path -> client_ids
        self.symbol_subscribers: Dict[str, Set[str]] = defaultdict(set)  # symbol_id -> client_ids
    ...
    async def add_dependency(self, source_symbol: Symbol, target_symbol: Symbol, 
                           dependency_type: DependencyType, file_path: str,
                           line_number: int = 0, column_number: int = 0) -> bool:
        ...
        try:
            ...
            if self._creates_cycle(source_id, target_id):
                ...
                logger.warning(f"Cyclic dependency detected: {source_id} -> {target_id}")
            
            self.metrics["total_dependencies"] += 1
            self.metrics["dependency_updates"] += 1
            
            logger.debug(f"Added dependency: {source_symbol.name} -> {target_symbol.name} ({dependency_type})")
            return True
    ...
    async def analyze_symbol_impact(self, symbol_id: str, change_type: str = "modified") -> ImpactAnalysis:
        ...
        try:
            ...
            while queue and depth < self.max_analysis_depth:
                ...
                for _ in range(level_size):
                    ...
                    next_dependents = self.reverse_dependency_graph.get(current_symbol, set())
            ...
            if change_type in ["deleted", "signature_changed"]:
                ...
                breaking_changes = [
                    dep for dep in directly_affected
            ...
            logger.debug(f"Impact analysis for {symbol_id}: {len(directly_affected)} direct, "
    ...
    async def find_dependency_path(self, source_symbol_id: str, target_symbol_id: str) -> Optional[DependencyPath]:
        """Find dependency path between two symbols"""
        try:
            if source_symbol_id not in self.symbols or target_symbol_id not in self.symbols:
                return None
            ...
            while queue:
                ...
                for dependency_id in self.dependency_graph.get(current_symbol, set()):
                    if dependency_id not in visited and len(path) < self.max_analysis_depth:
                        visited.add(dependency_id)
                        
                        # Find dependency type
                        dep_type = self._get_dependency_type(current_symbol, dependency_id)
    ...
    async def get_symbol_dependencies(self, symbol_id: str, depth: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            result["direct_dependencies"] = [
                {
                    "symbol_id": dep_id,
                    "symbol": self.symbols[dep_id].__dict__ if dep_id in self.symbols else None,
    ...
    async def process_file_changes(self, file_changes: List[FileChange]) -> List[ImpactAnalysis]:
            ...
            for file_change in file_changes:
                ...
                if file_change.change_type == ChangeType.DELETED:
                    ...
                    for symbol_id in symbols_in_file:
                        analysis = await self.analyze_symbol_impact(symbol_id, "deleted")
                    ...
                    for symbol_id in symbols_in_file:
                        analysis = await self.analyze_symbol_impact(symbol_id, "modified")
    ...
    def _generate_change_id(self) -> str:
        ...
        return f"change_{timestamp}_{hashlib.md5(str(timestamp).encode()).hexdigest()[:8]}"
    ...
    def _calculate_impact_score(self, direct_count: int, indirect_count: int, 
                              breaking_count: int, symbol_type: SymbolType, 
    ...
    def _generate_impact_suggestions(self, symbol: SymbolNode, change_type: str,
                                   affected_symbols: List[str], breaking_changes: List[str]) -> List[str]:
        """Generate suggestions for handling symbol impact"""
        suggestions = []
        
        if breaking_changes:
            suggestions.append(f"Consider gradual migration for {len(breaking_changes)} breaking changes")
            suggestions.append("Add deprecation warnings before removing symbols")
        
        if len(affected_symbols) > 10:
            suggestions.append("Consider batch refactoring tools for large-scale changes")
        ...
        try:
            while True:
                if self.pending_analysis:
                    ...
                    batch = []
                    for _ in range(min(self.batch_processing_size, len(self.pending_analysis))):
                        if self.pending_analysis:
                    ...
                    for symbol_id in batch:
                        if symbol_id in self.symbols:
                            ...
                            if analysis.impact_score > self.impact_threshold:
                                await self._notify_symbol_subscribers(symbol_id, analysis)
    ...
    async def _periodic_cleanup(self):
        ...
        try:
            while True:
                ...
                self.symbol_changes = [
                    change for change in self.symbol_changes
    ...
    async def _notify_symbol_subscribers(self, symbol_id: str, analysis: ImpactAnalysis):
        ...
        try:
            ...
            if subscribers:
                ...
                logger.debug(f"Notified {len(subscribers)} subscribers about changes to {symbol_id}")
    ...
    def _get_dependency_type(self, source_id: str, target_id: str) -> Optional[DependencyType]:
        """Get dependency type between two symbols"""
        for edge_id, edge in self.dependencies.items():
            if edge.source_symbol_id == source_id and edge.target_symbol_id == target_id:
                return edge.dependency_type
        return None
    
    def _calculate_path_strength(self, path: List[str], path_types: List[DependencyType]) -> float:
    ...
    async def _build_dependency_tree(self, symbol_id: str, remaining_depth: int, 
        ...
        for dep_id in dependencies:
            if dep_id not in visited:
                subtree = await self._build_dependency_tree(dep_id, remaining_depth - 1, visited.copy())
                tree[dep_id] = {
                    "symbol": self.symbols[dep_id].__dict__ if dep_id in self.symbols else None,
                    "dependency_type": self._get_dependency_type(symbol_id, dep_id),
