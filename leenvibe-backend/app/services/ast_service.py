...
from ..models.ast_models import (
    FileAnalysis, ProjectIndex, Symbol, Dependency, ComplexityMetrics,
    SymbolType, LanguageType, ASTNode, Reference, CallGraph, DependencyGraph
)
...
class ASTAnalysisService:
            ...
            for child in node.children:
                if child.type == 'identifier':
                    ...
                    return Symbol(
                        ...
                        column_start=node.start_point[1],
                        column_end=node.end_point[1],
                        signature=tree_sitter_manager.get_node_text(node, source_bytes)[:100]
                    )
    ...
    async def _create_js_function_symbol(self, node, source_bytes: bytes, file_path: str) -> Optional[Symbol]:
        ...
        try:
            ...
            # Extract parameters
            parameters = []
            param_nodes = tree_sitter_manager.find_nodes_by_type(node, ['formal_parameter', 'identifier'])
            for param_node in param_nodes:
                param_name = tree_sitter_manager.get_node_text(param_node, source_bytes)
    ...
    async def _calculate_complexity(
        ...
        try:
            ...
            functions = tree_sitter_manager.find_nodes_by_type(
                tree.root_node, ['function_definition', 'function_declaration', 'method_definition']
            )
            ...
            if metrics.lines_of_code > 0:
                metrics.maintainability_index = max(0, 
                    171 - 5.2 * (metrics.cyclomatic_complexity / metrics.number_of_functions if metrics.number_of_functions > 0 else 0)
                    ...
                    for stmt in child.children:
                        if stmt.type == 'expression_statement':
                            for expr in stmt.children:
                                if expr.type == 'string':
                                    docstring = tree_sitter_manager.get_node_text(expr, source_bytes)
    ...
    def _is_top_level_node(self, node) -> bool:
        ...
        parent = node.parent
        while parent:
            if parent.type in ['function_definition', 'class_definition', 'method_definition']:
                return False
