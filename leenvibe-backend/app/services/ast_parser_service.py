...
class TreeSitterService:
    ...
    def __init__(self):
        self.parsers: Dict[str, Any] = {}  # Using Any instead of Parser for compatibility
        self.languages: Dict[str, Any] = {}  # Using Any instead of Language for compatibility
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.is_initialized = True
            logger.info(f"Tree-sitter service initialized with {len(available_languages)} languages: {available_languages}")
            return True
    ...
    async def _basic_code_analysis(self, file_path: str, content: str, language: str) -> CodeStructure:
    ...
    def _analyze_python_code(self, lines: List[str]) -> tuple[List[CodeSymbol], List[str], Set[str]]:
        ...
        for i, line in enumerate(lines):
            ...
            # Function definitions
                ...
                symbols.append(CodeSymbol(
                    ...
                    type='function',
                    start_line=i + 1,
                    end_line=i + 1,  # We'd need proper parsing for accurate end line
                    start_byte=0,
                    end_byte=len(line),
    ...
    def _analyze_javascript_code(self, lines: List[str]) -> tuple[List[CodeSymbol], List[str], Set[str]]:
    ...
    def _analyze_swift_code(self, lines: List[str]) -> tuple[List[CodeSymbol], List[str], Set[str]]:
    ...
    def _calculate_complexity_score(self, lines: List[str], symbols: List[CodeSymbol]) -> float:
        ...
        control_keywords = ['if', 'else', 'elif', 'for', 'while', 'try', 'except', 'switch', 'case']
        for line in lines:
    ...
    async def get_file_summary(self, file_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "symbols": {
                "functions": len([s for s in structure.symbols if s.type == 'function']),
                "classes": len([s for s in structure.symbols if s.type in ['class', 'struct']]),
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analysis_level": "full_ast_parsing" if self.tree_sitter_available else "basic_text_parsing"
        }