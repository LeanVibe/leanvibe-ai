...
from ..models.ast_models import LanguageType, ASTNode
    ...
    def tree_to_ast_node(self, node: Node, source_code: bytes) -> ASTNode:
        ...
        try:
            ...
            return ASTNode(
                ...
                text=node_text[:1000] if len(node_text) > 1000 else node_text,  # Limit text length
                children=children
            )
    ...
    async def extract_imports(self, tree: tree_sitter.Tree, source_code: bytes, language: LanguageType) -> List[Dict]:
        ...
        try:
            if language == LanguageType.PYTHON:
                imports.extend(self._extract_python_imports(tree.root_node, source_code))
