...
class SmartCacheWarmingService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"Cache warming service initialized with {len(self.project_stats)} tracked projects")
            return True
        ...
        try:
            ...
            if project_path in self.project_stats:
                ...
                if stats.access_count > 0:
                    stats.average_session_duration = stats.total_session_time / stats.access_count
                
                # Recalculate warming score
                self._update_warming_score(project_path)
            
            logger.debug(f"Tracked session end for {project_path}: {session_duration:.1f}s")
    ...
    def _update_warming_score(self, project_path: str):
        ...
        try:
            ...
            session_quality_score = min(1.0, stats.total_session_time / min_quality_time)
            
            # Activity score bonus for files/symbols/analysis
            activity_bonus = min(0.2, (stats.files_accessed + stats.symbols_queried + stats.analysis_requests) / 100)
            ...
            logger.debug(f"Updated warming score for {project_path}: {warming_score:.3f}")
            
        except Exception as e:
            logger.error(f"Error updating warming score: {e}")
    
    async def queue_warming_task(self, project_path: str, priority_override: Optional[float] = None):
        ...
        try:
            ...
            priority_score = priority_override or self.project_stats[project_path].warming_score
            ...
            logger.info(f"Queued warming task for {project_path} with priority {priority_score:.3f}")
            return True
    ...
    def _is_eligible_for_warming(self, project_path: str) -> bool:
        ...
        try:
            ...
            return (
                stats.access_count >= strategy.min_access_count and
                stats.total_session_time >= strategy.min_total_session_time and
                stats.warming_score > 0.3  # Minimum warming score threshold
            )
    ...
    async def _start_background_tasks(self):
        ...
        try:
            ...
            self.warming_background_task = asyncio.create_task(self._warming_processor())
    ...
    async def _process_warming_queue(self):
        ...
        try:
            ...
            tasks_to_start = min(
                len(self.warming_queue),
                strategy.max_concurrent_warming - active_count
            )
            ...
            if project_index:
                ...
                if task.project_path in self.project_stats:
                    ...
                    self.project_stats[task.project_path].warming_success_rate = min(
                        1.0, self.project_stats[task.project_path].warming_success_rate + 0.1
                    )
                
                logger.info(f"Successfully warmed cache for {task.project_path}: {task.files_warmed} files")
            else:
    ...
    async def _warming_task_completed(self, task: WarmingTask, async_task: asyncio.Task):
        ...
        try:
            ...
            logger.info(f"Warming task completed for {task.project_path}: "
    ...
    async def _cleanup_old_statistics(self):
        ...
        try:
            ...
            if projects_to_remove:
                logger.info(f"Cleaned up {len(projects_to_remove)} old project statistics")
    ...
    async def _load_project_stats(self):
        ...
        try:
            ...
            for project_path, stats_data in data.items():
                ...
                stats_data["first_access"] = datetime.fromisoformat(stats_data["first_access"])
                stats_data["last_access"] = datetime.fromisoformat(stats_data["last_access"])
                
                if stats_data.get("last_warmed"):
                    stats_data["last_warmed"] = datetime.fromisoformat(stats_data["last_warmed"])
    ...
    def get_warming_candidates(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for project_path, stats in self.project_stats.items():
                if (self._is_eligible_for_warming(project_path) and 
                    not self._is_recently_warmed(project_path)):
