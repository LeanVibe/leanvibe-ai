...
class CascadingImpact:
    ...
    origin_project: str
    change_type: str
    affected_projects: Dict[str, List[str]] = field(default_factory=dict)  # project -> symbol_ids
    propagation_paths: List[List[str]] = field(default_factory=list)  # Paths of propagation
...
class ProjectDependencyGraph:
    ...
    projects: Dict[str, Set[str]] = field(default_factory=lambda: defaultdict(set))  # project -> dependent projects
    reverse_dependencies: Dict[str, Set[str]] = field(default_factory=lambda: defaultdict(set))  # project -> projects that depend on it
    boundaries: Dict[str, ProjectBoundary] = field(default_factory=dict)
    external_dependencies: Dict[str, Set[str]] = field(default_factory=lambda: defaultdict(set))  # project -> external deps
...
class CascadingImpactAnalyzer:
    ...
    async def register_project_boundary(
        ...
        try:
            ...
            if boundary_type == ProjectBoundaryType.EXTERNAL:
                self.project_graph.external_dependencies[source_project].add(target_project)
            
            logger.debug(f"Registered project boundary: {boundary_id} ({boundary_type})")
            return True
    ...
    async def analyze_cascading_impact(
        ...
        try:
            ...
            logger.info(f"Cascading impact analysis complete: {len(impact.affected_projects)} projects, "
    ...
    async def _analyze_propagation(
        ...
        while queue and len(visited_projects) < 100:  # Limit scope
            ...
            for dependent_info in deps.get("direct_dependents", []):
                ...
                if not include_external and self._is_external_project(dependent_project):
                    continue
        ...
        try:
            ...
            for file_path, file_analysis in project_index.files.items():
                for symbol in file_analysis.symbols:
                    ...
                    symbol_deps = await symbol_dependency_tracker.get_symbol_dependencies(
                    ...
                    for dep_info in symbol_deps.get("direct_dependencies", []):
                        ...
                        if dep_project != project_id:
                            cross_dep = CrossProjectDependency(
                                ...
                                dependency_type=dep_info.get("dependency_type", DependencyType.UNKNOWN),
                                propagation_type=ImpactPropagation.DIRECT
                            )
                            
                            dependencies[dep_project].append(cross_dep)
                            self.cross_project_dependencies[cross_dep.dependency_id] = cross_dep
    ...
    def _get_symbol_project(self, file_path: str) -> str:
        ...
        if len(path.parts) > 0:
            ...
            if len(directories) == 0 and len(path.parts) > 0:
                ...
                if '.' not in first_part or not first_part.endswith(('.py', '.js', '.ts', '.swift')):
                    return first_part
    ...
    def _is_breaking_change(self, change_type: str, dependency_info: Dict[str, Any]) -> bool:
    ...
    def _generate_recommendations(self, impact: CascadingImpact) -> List[str]:
        ...
        if impact.breaking_changes:
            recommendations.append(f"Create migration guide for {len(impact.breaking_changes)} breaking changes")
            recommendations.append("Consider providing compatibility layer")
        
        # Communication recommendations
        if len(impact.affected_projects) > 5:
            recommendations.append("Send advance notice to dependent project maintainers")
            recommendations.append("Create detailed changelog with migration instructions")
    ...
    def _generate_impact_id(self) -> str:
        ...
        return f"impact_{timestamp}_{hashlib.md5(str(timestamp).encode()).hexdigest()[:8]}"
    ...
    def _update_metrics(self, impact: CascadingImpact, analysis_time: float):
        ...
        new_avg_depth = (current_avg_depth * (self.metrics["total_analyses"] - 1) + 
                        impact.max_propagation_depth) / self.metrics["total_analyses"]
        self.metrics["average_propagation_depth"] = new_avg_depth
        
        current_avg_time = self.metrics["average_analysis_time_ms"]
        new_avg_time = (current_avg_time * (self.metrics["total_analyses"] - 1) + 
                       analysis_time * 1000) / self.metrics["total_analyses"]
