...
from ..models.event_models import (
    EventData, EventType, EventPriority, NotificationChannel,
    ClientPreferences, NotificationBatch, StreamingMessage,
    ConnectionState, EventStats
)
...
class EventFilter:
    ...
    def should_deliver(self, event: EventData, preferences: ClientPreferences) -> bool:
        ...
        if (NotificationChannel.ALL not in preferences.enabled_channels and
            event.channel not in preferences.enabled_channels):
            return False
        ...
        # Check rate limiting
        if not self._check_rate_limit(preferences.client_id, preferences.max_events_per_second):
            return False
    ...
    def _apply_custom_filters(self, event: EventData, custom_filters: Dict[str, Any]) -> bool:
...
class EventBatcher:
    ...
    async def add_event(self, client_id: str, event: EventData, preferences: ClientPreferences):
        ...
        if client_id not in self.batch_timers:
            self.batch_timers[client_id] = asyncio.create_task(
                self._flush_after_delay(client_id, preferences.batch_interval_ms / 1000.0)
            )
...
class EventStreamingService:
    ...
    def register_client(self, client_id: str, websocket, preferences: Optional[ClientPreferences] = None):
    ...
    async def _deliver_to_client(self, client_id: str, event: EventData, client_state: ConnectionState):
        ...
        try:
            ...
            if len(events_to_send) == 1:
                message = self._create_streaming_message(events_to_send[0], client_state)
            else:
    ...
    def _create_streaming_message(self, event: EventData, client_state: ConnectionState) -> StreamingMessage:
    ...
    def _create_batch_message(self, events: List[EventData], client_state: ConnectionState) -> StreamingMessage:
        """Create a batch streaming message"""
        client_state.sequence_number += 1
        
        # Use the highest priority event for the batch
        max_priority = max(events, key=lambda e: self._priority_value(e.priority)).priority
    ...
    async def _send_to_websocket(self, client_id: str, message: StreamingMessage, client_state: ConnectionState):
        ...
        try:
            ...
            if client_state.preferences.enable_compression:
                message_bytes, compressed = self.compression_manager.compress_message(message_json)
...
async def emit_file_change(file_path: str, change_type: str, old_path: Optional[str] = None):
...
async def emit_analysis_completed(analysis_type: str, file_path: str, processing_time: float):
...
async def emit_violation_detected(violation_id: str, violation_type: str, severity: str, 
                                file_path: str, description: str):
    """Emit a violation detection event"""
    from ..models.event_models import create_violation_event
    event = create_violation_event(violation_id, violation_type, severity, file_path, description)
    await event_streaming_service.emit_event(event)


async def emit_agent_event(session_id: str, event_type: EventType, query: Optional[str] = None):
