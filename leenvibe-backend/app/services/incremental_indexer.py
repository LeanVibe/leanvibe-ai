...
class IncrementalProjectIndexer:
    ...
    async def get_or_create_project_index(
        ...
        try:
            ...
            needs_full_reindex = (
                force_full_reindex or
                cache is None or
                self._needs_full_reindex(cache)
            )
    ...
    async def update_from_file_changes(
        ...
        try:
            ...
            indexing_time = time.time() - start_time
            logger.info(f"Incremental update completed in {indexing_time:.2f}s "
        ...
        try:
            ...
            for file_path, analysis in project_index.files.items():
                file_entries[file_path] = FileIndexEntry(
                    ...
                    parsing_errors=len(analysis.parsing_errors) if analysis.parsing_errors else 0
                )
            ...
            cache = IncrementalIndexCache(
                ...
                dependency_graph_hash=self._calculate_object_hash(project_index.dependencies),
    ...
    def _needs_full_reindex(self, cache: IncrementalIndexCache) -> bool:
        ...
        try:
            ...
            for path in workspace.rglob('*'):
                if path.is_file() and path.suffix.lower() in {'.py', '.js', '.ts', '.jsx', '.tsx', '.swift'}:
            ...
            if change_ratio > 0.3:  # More than 30% files changed
                logger.info(f"Significant file changes ({change_ratio:.1%}), needs full reindex")
                return True
    ...
    async def _incremental_update(
        ...
        try:
            ...
            logger.info(f"Incremental update: {len(files_to_analyze)} files to analyze, "
            ...
            for file_path in removed_files:
                if file_path in project_index.files:
                    ...
                    # Update counters
                    project_index.supported_files = max(0, project_index.supported_files - 1)
            ...
            return await self._full_reindex(workspace_path, include_patterns, exclude_patterns)
    ...
    async def _cache_to_project_index(self, cache: IncrementalIndexCache) -> ProjectIndex:
        """Convert cache to project index"""
        try:
            project_index = ProjectIndex(
                workspace_path=cache.project_path,
                last_indexed=cache.updated_at
            )
            
            # Set metadata
            project_index.total_files = cache.project_metadata.get("total_files", 0)
            project_index.supported_files = cache.project_metadata.get("supported_files", 0)
            project_index.parsing_errors = cache.project_metadata.get("parsing_errors", 0)
            ...
            return ProjectIndex(workspace_path=cache.project_path, last_indexed=time.time())
    ...
    async def _analyze_files_batch(self, file_paths: List[str]) -> Dict[str, FileAnalysis]:
