...
class IncrementalGraphUpdateService:
    ...
    async def process_file_changes(
        ...
        try:
            if not graph_service.initialized:
                logger.warning("Graph service not initialized, skipping incremental update")
                return None
            ...
            for change in file_changes:
                ...
                if change.change_type == ChangeType.DELETED:
                    ...
                    changes.extend(await self._build_deletion_changes(project_id, file_path))
                    ...
                    if file_path in file_analyses:
                        ...
                        changes.extend(await self._build_update_changes(
                            project_id, file_path, analysis, 
                            is_new=(change.change_type == ChangeType.CREATED)
                        ))
                    else:
                        # Basic file node update without detailed analysis
                        changes.extend(await self._build_basic_file_update(
                            project_id, file_path, change.change_type == ChangeType.CREATED
                        ))
                
                elif change.change_type in [ChangeType.MOVED, ChangeType.RENAMED]:
                    # Handle file move/rename
                    old_path = change.old_path or file_path
                    changes.extend(await self._build_move_changes(
                        project_id, old_path, file_path, 
            ...
            if success:
                ...
                logger.info(f"Graph update completed in {execution_time_ms:.1f}ms: "
        ...
        try:
            ...
            for symbol in analysis.symbols:
                ...
                if symbol.id not in existing_symbol_ids:
                    ...
                    old_symbol_data = next((s for s in existing_symbols if s['id'] == symbol.id), None)
                    if old_symbol_data and self._has_symbol_changed(old_symbol_data, symbol_data):
                        changes.append(GraphChange(
            ...
            changes.extend(await self._build_dependency_changes(
    ...
    async def _apply_update_batch(self, batch: GraphUpdateBatch) -> bool:
        ...
        try:
            ...
            if rel_deletions:
                await self._apply_relationship_deletions(batch.project_id, rel_deletions)
            ...
            if rel_additions:
                await self._apply_relationship_additions(batch.project_id, rel_additions)
    ...
    async def _apply_node_additions(self, project_id: str, changes: List[GraphChange]):
        ...
        for batch_changes in self._batch_changes(changes, self.batch_size):
            ...
            for change in batch_changes:
                by_type[change.entity_type].append({
    ...
    async def _apply_relationship_additions(self, project_id: str, changes: List[GraphChange]):
    ...
    async def _apply_relationship_updates(self, project_id: str, changes: List[GraphChange]):
    ...
    async def _apply_relationship_deletions(self, project_id: str, changes: List[GraphChange]):
    ...
    async def _get_node_data(self, project_id: str, node_id: str, node_type: str) -> Optional[Dict]:
    ...
    async def _get_file_dependencies(self, project_id: str, file_path: str) -> List[Dict]:
    ...
    def _update_metrics(self, batch: GraphUpdateBatch, propagation: PropagationResult):
        ...
        if propagation.propagation_depth > self.metrics["max_propagation_depth_reached"]:
            self.metrics["max_propagation_depth_reached"] = propagation.propagation_depth
    
    async def rollback_batch(self, batch_id: str) -> bool:
        """Rollback a previously applied batch"""
        try:
            # Find batch in history
            batch = next((b for b in self.update_history if b.batch_id == batch_id), None)
            
            if not batch or not batch.success:
                logger.warning(f"Cannot rollback batch {batch_id}: not found or not successful")
                return False
            ...
            for change in reversed(batch.changes):  # Reverse order
                if change.update_type == GraphUpdateType.NODE_ADDED:
                    ...
                    reverse_changes.append(GraphChange(
