...
from .symbol_dependency_tracker import (
    symbol_dependency_tracker, SymbolNode, DependencyType,
    ImpactAnalysis
)
...
class ChangeClassifier:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Change classifier initialized with {len(self.patterns)} patterns and {len(self.rules)} rules")
            return True
    ...
    async def classify_change(
        ...
        try:
            ...
            if not self._is_code_file(file_change.file_path):
                classification.category = self._get_non_code_category(file_change.file_path)
            ...
            if file_change.change_type == ChangeType.DELETED:
                ...
                await self._analyze_modification(classification, file_change, old_content, new_content)
            ...
            logger.debug(f"Classified change {file_change.file_path}: {classification.risk_level} "
        ...
        try:
            ...
            async def classify_single(change):
                async with semaphore:
                    ...
                    if content_provider:
                        old_content = await content_provider.get_old_content(change.file_path)
                        new_content = await content_provider.get_new_content(change.file_path)
            ...
            for i, result in enumerate(classifications):
                if isinstance(result, Exception):
                    logger.error(f"Error classifying change {changes[i].file_path}: {result}")
    ...
    def get_classification_summary(
        ...
        try:
            ...
            breaking_count = (
                risk_counts[ChangeRisk.BREAKING] + 
                risk_counts[ChangeRisk.HIGH_RISK]
            )
            safe_count = (
                risk_counts[ChangeRisk.SAFE] + 
                risk_counts[ChangeRisk.LOW_RISK]
            )
            ...
            return {
                ...
                "high_confidence_changes": len([c for c in classifications if c.confidence_score > 0.8]),
                "low_confidence_changes": len([c for c in classifications if c.confidence_score < 0.5])
            }
        ...
        try:
            ...
            self.rules.extend([
                ...
                ClassificationRule(
                    ...
                    conditions=[
                        "file_path matches test patterns"
                    ],
                    classification=ChangeRisk.SAFE,
                    confidence=0.9,
                    priority=5
                ...
                ClassificationRule(
                    ...
                    conditions=[
                        "file_path matches doc patterns"
                    ],
                    classification=ChangeRisk.SAFE,
                    confidence=0.95,
                    priority=4
    ...
    def _determine_category(self, file_change: FileChange) -> ChangeCategory:
        ...
        if any(pattern in file_path for pattern in ['.md', '.rst', '.txt', 'readme', 'changelog', 'docs/']):
        ...
        if any(pattern in file_path for pattern in [
            ...
            '.yml', '.yaml', '.json', '.toml', '.ini', '.cfg', '.config'
        ]):
            return ChangeCategory.BUILD_CHANGE
        
        # Test changes (after config checks)
        if any(pattern in file_path for pattern in ['test', 'spec', '__test__', '.test.', '.spec.']):
    ...
    def _is_code_file(self, file_path: str) -> bool:
        ...
        code_extensions = {'.py', '.js', '.ts', '.tsx', '.jsx', '.java', '.c', '.cpp', '.h', '.swift', '.go', '.rs'}
    ...
    async def _analyze_deletion(self, classification: ChangeClassification, file_change: FileChange):
        ...
        if 'api' in file_change.file_path.lower() or 'public' in file_change.file_path.lower():
    ...
    async def _analyze_creation(self, classification: ChangeClassification, file_change: FileChange, content: Optional[str]):
    ...
    async def _analyze_modification(
        ...
        if old_content and new_content:
            ...
            await self._analyze_content_changes(classification, old_content, new_content)
        
        # Check file type specific patterns
        await self._apply_pattern_analysis(classification, file_change)
    
    async def _analyze_move(self, classification: ChangeClassification, file_change: FileChange):
        ...
        if 'api' in file_change.file_path.lower():
            classification.risk_level = ChangeRisk.HIGH_RISK
            classification.breaking_changes.append(BreakingChangeType.API_CONTRACT_CHANGE)
        ...
        try:
            ...
            for line in removed_lines:
                ...
                if self._is_function_signature_line(line):
                    classification.risk_level = ChangeRisk.HIGH_RISK
                    classification.breaking_changes.append(BreakingChangeType.SIGNATURE_CHANGE)
    ...
    async def _apply_pattern_analysis(self, classification: ChangeClassification, file_change: FileChange):
    ...
    async def _apply_classification_rules(self, classification: ChangeClassification, file_change: FileChange):
        """Apply classification rules"""
        try:
            for rule in self.rules:
                if self._rule_applies(rule, classification, file_change):
                    ...
                    if rule.classification == ChangeRisk.BREAKING:
                        classification.risk_level = ChangeRisk.BREAKING
                        classification.compatibility = CompatibilityLevel.BREAKING
                    elif rule.classification == ChangeRisk.HIGH_RISK and classification.risk_level != ChangeRisk.BREAKING:
                        classification.risk_level = ChangeRisk.HIGH_RISK
                    elif rule.classification == ChangeRisk.SAFE and classification.risk_level == ChangeRisk.MEDIUM_RISK:
                        classification.risk_level = ChangeRisk.SAFE
                        classification.compatibility = CompatibilityLevel.COMPATIBLE
                    
                    classification.breaking_changes.extend(rule.breaking_types)
                    classification.reasons.append(f"Rule applied: {rule.name}")
                    classification.confidence_score = max(classification.confidence_score, rule.confidence)
    ...
    def _rule_applies(self, rule: ClassificationRule, classification: ChangeClassification, file_change: FileChange) -> bool:
    ...
    def _evaluate_condition(self, condition: str, classification: ChangeClassification, file_change: FileChange) -> bool:
        ...
        if "change_type == DELETED" in condition:
            ...
            return any(ext in file_change.file_path.lower() for ext in ['.md', '.rst', 'doc'])
    ...
    def _finalize_classification(self, classification: ChangeClassification):
        ...
        if classification.breaking_changes:
            classification.migration_suggestions = self._generate_migration_suggestions(classification)
    
    def _generate_migration_suggestions(self, classification: ChangeClassification) -> List[str]:
    ...
    def _is_public_api_line(self, line: str) -> bool:
        ...
        line = line.strip()
        return any(keyword in line for keyword in ['def ', 'class ', 'export ', 'public '])
    
    def _is_function_signature_line(self, line: str) -> bool:
        """Check if line is a function signature"""
        line = line.strip()
        return bool(re.match(r'def\s+\w+\s*\(|function\s+\w+\s*\(|\w+\s*\([^)]*\)\s*{', line))
    ...
    def _is_visibility_reduction(self, old_visibility: str, new_visibility: str) -> bool:
    ...
    def _has_parameter_type_changes(self, old_params: List[str], new_params: List[str]) -> bool:
