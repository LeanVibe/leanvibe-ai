...
class VectorStoreService:
    ...
    def _create_basic_embedding(self, content: str) -> List[float]:
        ...
        keywords = ['function', 'class', 'import', 'def', 'const', 'let', 'var', 'struct']
    ...
    async def add_code_embedding(self, embedding: CodeEmbedding) -> bool:
        ...
        try:
            ...
            logger.debug(f"Added embedding for {embedding.symbol_name} in {embedding.file_path}")
            return True
        ...
        try:
            ...
            for symbol in code_structure.symbols:
                ...
                symbol_embedding = CodeEmbedding(
                    id=self._generate_content_hash(f"{file_path}:{symbol.name}:{symbol.start_line}"),
    ...
    async def search_similar_code(self, query: str, n_results: int = 5, 
                                 ...
                                 symbol_type_filter: Optional[str] = None) -> List[SearchResult]:
        ...
        try:
            ...
            if self.chromadb_available and self.collection:
                # Use ChromaDB search
                return await self._search_chromadb(query_embedding, n_results, file_filter, symbol_type_filter)
            else:
                # Use mock search
                return await self._search_mock(query, n_results, file_filter, symbol_type_filter)
            
        except Exception as e:
            logger.error(f"Error searching similar code: {e}")
            return []
    
    async def _search_chromadb(self, query_embedding: List[float], n_results: int,
                              file_filter: Optional[str], symbol_type_filter: Optional[str]) -> List[SearchResult]:
    ...
    async def _search_mock(self, query: str, n_results: int,
                          file_filter: Optional[str], symbol_type_filter: Optional[str]) -> List[SearchResult]:
