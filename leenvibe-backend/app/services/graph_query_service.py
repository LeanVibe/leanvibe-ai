...
from ..models.graph_models import (
    GraphQuery, ImpactAnalysisResult, ArchitecturePattern,
    GraphVisualizationData, RelationshipType
)
...
class GraphQueryService:
    ...
    async def find_call_chains(
        ...
        try:
            ...
            with self.graph_service.driver.session(database=self.graph_service.database) as session:
                result = session.run(
                    query, 
    ...
    async def analyze_coupling(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.graph_service.driver.session(database=self.graph_service.database) as session:
                ...
                avg_coupling = total_coupling / len(coupling_data) if coupling_data else 0
                
                # Identify high coupling files (above average)
                high_coupling_files = [
                    f for f in coupling_data 
                    if f["total_coupling"] > avg_coupling * 1.5
                ]
            ...
            with self.graph_service.driver.session(database=self.graph_service.database) as session:
                ...
                for record in result:
                    dead_code.append({
                        ...
                        "confidence": self._calculate_dead_code_confidence(record["symbol_name"])
                    })
    ...
    def _calculate_dead_code_confidence(self, symbol_name: str) -> float:
        ...
        if any(pattern in symbol_name.lower() for pattern in [
            'test', 'example', 'demo', 'util', 'helper', 'config'
        ]):
            confidence *= 0.6
    ...
    async def suggest_refactoring_opportunities(self, project_id: str) -> List[Dict[str, Any]]:
            ...
            with self.graph_service.driver.session(database=self.graph_service.database) as session:
                ...
                for record in result:
                    similar_functions.append({
                        "function1": {"name": record["name1"], "file": record["file1"]},
                        "function2": {"name": record["name2"], "file": record["file2"]},
            ...
            with self.graph_service.driver.session(database=self.graph_service.database) as session:
                ...
                for record in result:
                    ...
                    if (not func_name.startswith('_') and 
                        not func_name.startswith('get') and
                        not func_name.startswith('set')):
                ...
                return {
                    ...
                    "recommendations": self._generate_test_recommendations(len(untested_functions))
                }
    ...
    def _generate_test_recommendations(self, gap_count: int) -> List[str]:
        ...
        if gap_count == 0:
            ...
            recommendations.append("⚡ Minor test coverage gaps - consider adding a few more tests")
        elif gap_count < 15:
            recommendations.append("⚠️ Moderate test coverage gaps - prioritize testing key functions")
        else:
    ...
    async def find_hotspots(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with self.graph_service.driver.session(database=self.graph_service.database) as session:
                ...
                hotspots = []
                for record in result:
                    if record["connection_count"] > 3:  # Only include well-connected nodes
                        hotspots.append({
                            "symbol_name": record["symbol_name"],
                            "file_path": record["file_path"],
                            "symbol_types": record["symbol_types"],
                            ...
                            "risk_level": self._calculate_hotspot_risk(record["connection_count"])
                        })
