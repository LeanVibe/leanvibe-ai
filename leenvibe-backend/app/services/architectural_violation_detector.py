...
from ..models.ast_models import (
    ...
    SymbolType, Reference, LanguageType
)
...
class ComponentMetrics:
    ...
    complexity_metrics: Dict[str, float] = field(default_factory=dict)  # cyclomatic, cognitive
...
class ArchitecturalViolationDetector:
    ...
    def __init__(self):
        ...
        self.violation_subscribers: Dict[str, Set[str]] = defaultdict(set)  # client subscriptions
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.analysis_task = asyncio.create_task(self._background_analysis_processor())
            self.monitoring_task = asyncio.create_task(self._continuous_monitoring())
            
            logger.info(f"Architectural violation detector initialized with {len(self.rules)} rules")
            return True
    ...
    async def analyze_file(self, file_path: str, content: Optional[str] = None) -> List[ViolationInstance]:
        ...
        try:
            ...
            component_metrics = await self._calculate_component_metrics(file_path, file_analysis)
            self.component_metrics[file_path] = component_metrics
            
            # Apply architectural rules
            for rule in self.rules.values():
                if rule.enabled:
                    rule_violations = await self._apply_rule(rule, file_path, file_analysis, component_metrics)
                    violations.extend(rule_violations)
            
            # Check for circular dependencies
            circular_violations = await self._detect_circular_dependencies(file_path, file_analysis)
    ...
    async def analyze_project(self, project_id: str) -> ArchitecturalReport:
        ...
        try:
            ...
            quality_score = self._calculate_quality_score(all_violations, len(project_index.files))
            ...
            analysis_time = (time.time() - start_time) * 1000
            logger.info(f"Project analysis completed in {analysis_time:.2f}ms: "
    ...
    async def process_file_change(self, file_change: FileChange) -> List[ViolationInstance]:
        ...
        try:
            ...
            logger.error(f"Error processing file change for {file_change.file_path}: {e}")
            return []
    ...
    def get_violations_by_severity(self, severity: ViolationSeverity) -> List[ViolationInstance]:
    ...
    async def _calculate_component_metrics(self, file_path: str, file_analysis: FileAnalysis) -> ComponentMetrics:
        """Calculate metrics for a component"""
        try:
            # Size metrics
            total_lines = len(file_analysis.content.splitlines()) if hasattr(file_analysis, 'content') else 0
            ...
            return ComponentMetrics(
                ...
                size_metrics={
                    ...
                    "functions": len([s for s in file_analysis.symbols if s.symbol_type == SymbolType.FUNCTION]),
                    "classes": len([s for s in file_analysis.symbols if s.symbol_type == SymbolType.CLASS])
                },
                complexity_metrics={
                    "average_complexity": avg_complexity,
                    "max_complexity": max((getattr(s, 'complexity', 1) or 1 for s in file_analysis.symbols), default=0)
                },
                coupling_metrics={
            ...
            return ComponentMetrics(component_id=file_path, component_type="file", file_path=file_path)
    
    async def _apply_rule(self, rule: ArchitecturalRule, file_path: str, 
                         file_analysis: FileAnalysis, component_metrics: ComponentMetrics) -> List[ViolationInstance]:
        """Apply an architectural rule to a file"""
        violations = []
        
        try:
            if rule.violation_type == ViolationType.SIZE_VIOLATION:
                violations.extend(self._check_size_violations(rule, file_path, file_analysis, component_metrics))
            
            elif rule.violation_type == ViolationType.COMPLEXITY_VIOLATION:
                violations.extend(self._check_complexity_violations(rule, file_path, file_analysis))
            
            elif rule.violation_type == ViolationType.COUPLING_VIOLATION:
                violations.extend(self._check_coupling_violations(rule, file_path, component_metrics))
            
            elif rule.violation_type == ViolationType.LAYER_VIOLATION:
                violations.extend(await self._check_layer_violations(rule, file_path, file_analysis))
            
            elif rule.violation_type == ViolationType.NAMING_CONVENTION_VIOLATION:
                violations.extend(self._check_naming_violations(rule, file_path, file_analysis))
            
            elif rule.violation_type == ViolationType.ANTI_PATTERN:
                violations.extend(self._check_anti_patterns(rule, file_path, file_analysis, component_metrics))
    ...
    def _check_size_violations(self, rule: ArchitecturalRule, file_path: str,
                              file_analysis: FileAnalysis, component_metrics: ComponentMetrics) -> List[ViolationInstance]:
    ...
    def _check_complexity_violations(self, rule: ArchitecturalRule, file_path: str,
                                   file_analysis: FileAnalysis) -> List[ViolationInstance]:
    ...
    def _check_coupling_violations(self, rule: ArchitecturalRule, file_path: str,
                                  component_metrics: ComponentMetrics) -> List[ViolationInstance]:
        """Check for coupling violations"""
        violations = []
        
        total_coupling = component_metrics.dependencies_in + component_metrics.dependencies_out
    ...
    async def _check_layer_violations(self, rule: ArchitecturalRule, file_path: str,
                                    file_analysis: FileAnalysis) -> List[ViolationInstance]:
        ...
        for dependency in file_analysis.dependencies:
            ...
            if (current_layer in rule.layers_from and 
                target_layer in rule.layers_to):
                violations.append(ViolationInstance(
                    violation_id=self._generate_violation_id(),
                    rule_id=rule.rule_id,
                    violation_type=rule.violation_type,
                    ...
                    context={"target_file": target_file, "dependency_type": dependency.dependency_type}
                ))
        
        return violations
    
    def _check_naming_violations(self, rule: ArchitecturalRule, file_path: str,
                               file_analysis: FileAnalysis) -> List[ViolationInstance]:
    ...
    def _check_anti_patterns(self, rule: ArchitecturalRule, file_path: str,
                           file_analysis: FileAnalysis, component_metrics: ComponentMetrics) -> List[ViolationInstance]:
        ...
        if rule.rule_id == "god_class_antipattern":
            for symbol in file_analysis.symbols:
                if symbol.symbol_type == SymbolType.CLASS:
                    class_lines = symbol.line_end - symbol.line_start + 1
                    class_methods = len([s for s in file_analysis.symbols 
                                       if s.symbol_type == SymbolType.METHOD and 
                                       s.line_start >= symbol.line_start and 
                    ...
                    if class_lines > 200 and class_methods > 10:
                        violations.append(ViolationInstance(
                            ...
                            confidence_score=0.7,
                            context={"lines": class_lines, "methods": class_methods}
                        ))
        
        return violations
    
    async def _detect_circular_dependencies(self, file_path: str, file_analysis: FileAnalysis) -> List[ViolationInstance]:
        ...
        try:
            ...
            for dependency in file_analysis.dependencies:
                ...
                reverse_path = await symbol_dependency_tracker.find_dependency_path(target_file, file_path)
    ...
    def _generate_summary(self, violations: List[ViolationInstance], 
                         component_metrics: List[ComponentMetrics]) -> Dict[str, Any]:
        ...
        if component_metrics:
            avg_complexity = sum(m.complexity_metrics.get("average_complexity", 0) 
                               for m in component_metrics) / len(component_metrics)
            avg_coupling = sum(m.dependencies_in + m.dependencies_out 
                             for m in component_metrics) / len(component_metrics)
            avg_cohesion = sum(m.cohesion_score for m in component_metrics) / len(component_metrics)
    ...
    def _calculate_quality_score(self, violations: List[ViolationInstance], total_files: int) -> float:
    ...
    def _generate_recommendations(self, violations: List[ViolationInstance], 
        ...
        if summary["violations_by_type"].get(ViolationType.SIZE_VIOLATION, 0) > 5:
            recommendations.append("Consider refactoring large classes and functions into smaller components")
        
        if summary["violations_by_type"].get(ViolationType.COMPLEXITY_VIOLATION, 0) > 3:
            recommendations.append("Reduce code complexity using early returns, guard clauses, and method extraction")
        
        if summary["violations_by_type"].get(ViolationType.COUPLING_VIOLATION, 0) > 2:
            recommendations.append("Implement dependency injection and use interfaces to reduce coupling")
        
        if summary["violations_by_type"].get(ViolationType.LAYER_VIOLATION, 0) > 1:
            recommendations.append("Enforce architectural boundaries with clear layer separation")
        
        if summary["violations_by_severity"].get(ViolationSeverity.CRITICAL, 0) > 0:
            recommendations.append("Address critical violations immediately as they may impact system stability")
    ...
    def _remove_violations_for_file(self, file_path: str):
        ...
        # Remove from cache
        to_remove = [vid for vid, v in self.violation_cache.items() if v.file_path == file_path]
        for vid in to_remove:
            del self.violation_cache[vid]
    
    async def _notify_violation_subscribers(self, file_path: str, violations: List[ViolationInstance]):
        ...
        try:
            ...
            if subscribers:
                ...
                logger.debug(f"Notified {len(subscribers)} subscribers about {len(violations)} violations in {file_path}")
        except Exception as e:
    ...
    def _update_metrics(self, violations: List[ViolationInstance], analysis_time_ms: float):
        ...
        if self.metrics["analysis_time_ms"] == 0:
            ...
            self.metrics["analysis_time_ms"] = (self.metrics["analysis_time_ms"] + analysis_time_ms) / 2
    ...
    def _generate_report_id(self) -> str:
        ...
        timestamp = int(time.time() * 1000)
        return f"report_{timestamp}_{hashlib.md5(str(timestamp).encode()).hexdigest()[:8]}"
