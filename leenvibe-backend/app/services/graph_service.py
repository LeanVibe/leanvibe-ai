...
from ..models.graph_models import (
    GraphNode, GraphRelationship, ProjectNode, FileNode, SymbolNode,
    RelationshipType, NodeLabel, GraphQuery, GraphStatistics,
    ArchitecturePattern, ImpactAnalysisResult, GraphVisualizationData
)
from ..models.ast_models import ProjectIndex, Symbol, Dependency
...
class GraphService:
    ...
    async def store_project_graph(self, project_index: ProjectIndex, workspace_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Project graph stored successfully: {len(file_nodes)} files, {len(symbol_nodes)} symbols")
            return True
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                return ImpactAnalysisResult(
                    ...
                    affected_components={"direct": len(directly_affected), "indirect": len(indirectly_affected)},
                    recommendations=self._generate_impact_recommendations(risk_level, total_affected)
                )
    ...
    def _generate_impact_recommendations(self, risk_level: str, affected_count: int) -> List[str]:
        ...
        if risk_level == "high":
            ...
            recommendations.append("✅ Low impact change - standard testing should suffice")
    ...
    async def get_architecture_patterns(self, project_id: str) -> List[ArchitecturePattern]:
    ...
    async def _detect_mvc_pattern(self, project_id: str) -> Optional[ArchitecturePattern]:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                if len(mvc_files) >= 2:  # At least 2 components
                    controllers = [f for f in mvc_files if 'controller' in f["name"].lower()]
                    models = [f for f in mvc_files if 'model' in f["name"].lower()]
                    views = [f for f in mvc_files if 'view' in f["name"].lower()]
                    
                    component_count = len([c for c in [controllers, models, views] if c])
    ...
    async def _detect_service_layer_pattern(self, project_id: str) -> Optional[ArchitecturePattern]:
        """Detect Service Layer pattern"""
        try:
            query = """
            MATCH (p:Project {id: $project_id})-[:CONTAINS]->(f:File)
            ...
            with self.driver.session(database=self.database) as session:
                ...
                if len(service_files) >= 1:
                    ...
                    return ArchitecturePattern(
                        ...
                        relationships=["Controller -> Service", "Service -> Repository/DAO"]
                    )
    ...
    async def _detect_repository_pattern(self, project_id: str) -> Optional[ArchitecturePattern]:
        """Detect Repository pattern"""
        try:
            query = """
            MATCH (p:Project {id: $project_id})-[:CONTAINS]->(f:File)
    ...
    async def get_graph_statistics(self, project_id: Optional[str] = None) -> GraphStatistics:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                if total_nodes > 1:
                    ...
                    graph_density = total_relationships / max_edges if max_edges > 0 else 0.0
    ...
    async def get_visualization_data(self, project_id: str, max_nodes: int = 100) -> GraphVisualizationData:
