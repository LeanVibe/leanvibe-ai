...
class CachePersistenceService:
    ...
    async def save_cache(
        ...
        try:
            ...
            logger.info(
                f"Saving cache with {len(cache_data)} entries using {save_format}"
            )
            ...
            with self.persistence_lock:
                ...
                success = await self._save_cache_file(
                    cache_data, cache_file, save_format
                )
        ...
        try:
            ...
            with self.persistence_lock:
                ...
                self._update_load_metrics(
                    load_time, len(cache_data) if cache_data else 0
                )
    ...
    async def recover_cache(
        self, strategy: Optional[RecoveryStrategy] = None
    ) -> RecoveryResult:
        ...
        try:
            ...
            logger.info(
                f"Cache recovery completed: {result.recovered_entries}/{result.total_entries} entries recovered"
            )
    ...
    async def create_checkpoint(self, force: bool = False) -> Optional[CacheCheckpoint]:
        ...
        try:
            ...
            checkpoint = await self._create_checkpoint(
                self.cached_data, self.default_format
            )
                ...
                try:
                    ...
                    if not isinstance(data, dict):
                        ...
                        integrity_report["details"].append(
                            f"Invalid data structure for key: {key}"
                        )
                        continue
                    ...
                    if key not in self.cache_metadata:
                        ...
                        integrity_report["details"].append(
                            f"Missing metadata for key: {key}"
                        )
                    ...
                    integrity_report["corrupted_entries"] += 1
                    integrity_report["details"].append(
                        f"Validation error for key {key}: {str(e)}"
                    )
    ...
    async def cleanup_cache(self, max_age_days: int = 30) -> Dict[str, int]:
        ...
        try:
            ...
            for checkpoint in checkpoints_to_remove:
                checkpoint_file = (
                    self.cache_dir / "checkpoints" / f"{checkpoint.checkpoint_id}.cache"
                )
    ...
    async def _save_cache_file(
        self, data: Any, file_path: Path, format: CacheFormat
    ) -> bool:
    ...
    async def _load_cache_file(
        self, file_path: Path, format: CacheFormat
    ) -> Optional[Any]:
    ...
    async def _save_metadata(
        self, metadata: Dict[str, CacheMetadata], file_path: Path
    ) -> bool:
        """Save cache metadata to JSON file"""
        try:
            metadata_dict = {key: asdict(meta) for key, meta in metadata.items()}
    ...
    async def _load_metadata(
        self, file_path: Path
    ) -> Optional[Dict[str, CacheMetadata]]:
        ...
        try:
            ...
            for key, meta_dict in metadata_dict.items():
                ...
                if "last_accessed" in meta_dict:
                    meta_dict["last_accessed"] = datetime.fromisoformat(
                        meta_dict["last_accessed"]
                    )
                if "expires_at" in meta_dict and meta_dict["expires_at"]:
                    meta_dict["expires_at"] = datetime.fromisoformat(
                        meta_dict["expires_at"]
                    )
    ...
    async def _create_checkpoint(
        self, cache_data: Dict[str, Any], format: CacheFormat
    ) -> CacheCheckpoint:
    ...
    async def _full_recovery(self) -> RecoveryResult:
        ...
        for checkpoint in sorted(
            self.checkpoints, key=lambda cp: cp.timestamp, reverse=True
        ):
            checkpoint_file = (
                self.cache_dir / "checkpoints" / f"{checkpoint.checkpoint_id}.cache"
            )
            if checkpoint_file.exists():
                cache_data = await self._load_cache_file(
                    checkpoint_file, checkpoint.format
                )
                if cache_data and await self._verify_checksum(
                    cache_data, checkpoint.checksum
                ):
                    self.cached_data = cache_data
                    result.success = True
        ...
        if backup_dir.exists():
            for backup_file in sorted(backup_dir.glob("*_cache_data.*"), reverse=True):
                ...
                if format_ext in [f.value for f in CacheFormat]:
                    cache_data = await self._load_cache_file(
                        backup_file, CacheFormat(format_ext)
                    )
                    if cache_data:
                        self.cached_data = cache_data
                        result.success = True
            ...
            if checkpoint_file.exists():
                try:
                    ...
                    result.errors.append(
                        f"Error processing checkpoint {checkpoint.checkpoint_id}: {str(e)}"
                    )
    ...
    def _update_save_metrics(self, save_time_ms: float, entry_count: int):
        ...
        if self.metrics.average_save_time_ms == 0:
            ...
            self.metrics.average_save_time_ms = (
                self.metrics.average_save_time_ms + save_time_ms
            ) / 2
