...
class CacheInvalidationService:
    ...
    async def invalidate_file_cache(
        self, file_path: str, change_type: ChangeType, propagate: bool = True
        ...
        try:
            ...
            if len(self.invalidation_events) > self.max_events_history:
                self.invalidation_events = self.invalidation_events[
                    -self.max_events_history :
                ]

            invalidation_time = time.time() - start_time
            logger.debug(
                f"Cache invalidation completed: {len(invalidation_events)} files, "
    ...
    async def _propagate_invalidation(
        ...
        try:
            ...
            queue = deque(
                [(dep_file, current_depth + 1) for dep_file in node.dependents]
            )
    ...
    async def _invalidate_by_symbols(
        self, file_path: str, change_type: ChangeType, triggered_by: str
    ) -> List[InvalidationEvent]:
        ...
        try:
            ...
            for symbol in node.symbols:
                for using_file in self.symbol_to_files.get(symbol, set()):
                    if using_file != file_path and using_file not in [
    ...
    async def _execute_invalidation(self, event: InvalidationEvent):
        ...
        try:
            ...
            if hasattr(self, "_cache_handlers"):
                for handler in self._cache_handlers:
                    try:
                        ...
                        logger.warning(
                            f"Cache handler failed for {event.file_path}: {e}"
                        )
            ...
            logger.debug(
                f"Executed {event.invalidation_type} invalidation for {event.file_path}"
            )
        ...
        try:
            ...
            for file_path in changes_by_type.get(ChangeType.DELETED, []):
                if file_path not in processed_files:
                    events = await self.invalidate_file_cache(
                        file_path, ChangeType.DELETED, propagate=False
                    )
            ...
            for change_type in [
                ...
                for file_path in changes_by_type.get(change_type, []):
                    if file_path not in processed_files:
                        events = await self.invalidate_file_cache(
                            file_path, change_type, propagate=True
                        )
                        all_events.extend(events)
                        processed_files.add(file_path)
            ...
            logger.info(
                f"Batch invalidation: {len(file_changes)} changes -> {len(all_events)} invalidations"
            )
            return all_events
    ...
    async def optimize_graph(self):
        ...
        try:
            ...
            if removed_count > 0:
                logger.info(
                    f"Dependency graph optimized: removed {removed_count} stale nodes "
