...
from ..models.event_models import EventData, ConnectionState, EventType, EventPriority
...
class ReconnectionService:
    ...
    def register_client_session(self, client_id: str, connection_state: ConnectionState):
    ...
    async def client_reconnected(self, client_id: str, connection_state: ConnectionState) -> Dict[str, Any]:
        ...
        reconnection_info = {
            ...
            "disconnection_duration_ms": int((datetime.now() - session.last_seen).total_seconds() * 1000)
        }
        ...
        logger.info(f"Client {client_id} reconnected, replaying {len(missed_events_data)} missed events")
    ...
    def add_missed_event(self, client_id: str, event: EventData):
        ...
        if len(session.missed_events) > max_missed_events:
            ...
            logger.warning(f"Trimmed missed events for client {client_id} to {max_missed_events}")
    ...
    async def _heartbeat_monitor(self):
        ...
        while True:
            try:
                ...
                heartbeat_timeout = timedelta(milliseconds=self.config.heartbeat_interval_ms * 2)
    ...
    async def _cleanup_expired_sessions(self):
        ...
        while True:
            try:
                ...
                current_time = datetime.now()
                retention_period = timedelta(hours=self.config.missed_event_retention_hours)
                ...
                for session in self.client_sessions.values():
                    ...
                    session.missed_events = [
                        event for event in session.missed_events
                        if current_time - event.missed_at < retention_period
                    ]
                    
                    if len(session.missed_events) < original_count:
                        logger.info(f"Cleaned up {original_count - len(session.missed_events)} old missed events for {session.client_id}")
...
async def handle_client_reconnection(client_id: str, connection_state: ConnectionState) -> Dict[str, Any]:
