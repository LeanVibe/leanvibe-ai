...
def create_context_used(
    agent_response: dict, request: CodeCompletionRequest
) -> ContextUsed:
    ...
    return ContextUsed(
        ...
        language_detected=context_data.get(
...
async def code_completion(
    ...
    try:
        logger.info(
            f"Processing {request.intent} request for {request.file_path}:{request.cursor_position}"
        )
        ...
        try:
            ...
            if "invalid JSON" in error_msg or "parse" in error_msg.lower():
                ...
                logger.error(
                    f"Agent JSON parsing error for {request.intent} request: {e}"
                )
                return JSONResponse(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    content={
        ...
        response_data = {
            ...
            "suggestions": agent_response.get(
        ...
        if request.intent == "explain":
            response_data["explanation"] = agent_response.get(
                "explanation", agent_response.get("response", "")
            )
        elif request.intent == "refactor":
            response_data["refactoring_suggestions"] = agent_response.get(
                "refactoring_suggestions", agent_response.get("response", "")
            )
        elif request.intent == "debug":
            response_data["debug_analysis"] = agent_response.get(
                "debug_analysis", agent_response.get("response", "")
            )
        elif request.intent == "optimize":
            response_data["optimization_suggestions"] = agent_response.get(
                "optimization_suggestions", agent_response.get("response", "")
            )

        logger.info(
            f"Completed {request.intent} request in {processing_time:.1f}ms with confidence {response_data['confidence']:.2f}"
        )
        ...
        processing_time = (time.time() - start_time) * 1000
        logger.error(
            f"Unexpected error in code completion endpoint: {e}", exc_info=True
        )
