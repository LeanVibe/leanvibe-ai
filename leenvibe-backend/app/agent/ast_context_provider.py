...
class ASTContextProvider:
    ...
    async def get_file_context(self, file_path: str, cursor_position: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Generating AST context for {file_path} at position {cursor_position}")
            ...
            current_symbol = await self._get_symbol_at_position(file_path, cursor_position)
            ...
            context = {
                ...
                "file_analysis": {
                    ...
                    "imports": [self._dependency_to_dict(d) for d in file_analysis.dependencies],
                    "complexity": {
                ...
                # Current context
                "current_symbol": self._symbol_to_dict(current_symbol) if current_symbol else None,
                "surrounding_context": await self._get_surrounding_context(file_path, cursor_position),
                ...
                "architecture_insights": await self._get_architecture_context(file_path),
                
                # Suggestions context
                "suggestion_context": await self._build_suggestion_context(file_path, current_symbol)
            }
            ...
            if len(self.last_context_cache) > 10:
                oldest_key = min(self.last_context_cache.keys(), 
                               key=lambda k: self.last_context_cache[k][0])
                del self.last_context_cache[oldest_key]
            
            logger.info(f"Generated AST context with {len(context['file_analysis']['symbols'])} symbols")
            return context
    ...
    async def get_symbol_context(self, symbol_id: str, file_path: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            context = {
                ...
                "definition_count": len([ref for ref in references if ref.reference_type == "definition"]),
                
                # Relationships
                "related_symbols": [self._symbol_to_dict(s) for s in related_symbols],
                
                # Context analysis
                "importance_score": self._calculate_symbol_importance(symbol, references),
                "change_impact": await self._analyze_symbol_impact(symbol_id),
                
                # Suggestions
                "suggestions": await self._generate_symbol_suggestions(symbol, references)
            }
    ...
    async def get_completion_context(self, file_path: str, cursor_position: int, intent: str = "suggest") -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.info(f"Building completion context for {intent} at {file_path}:{cursor_position}")
            ...
            completion_context = {
                ...
                "current_file": {
                    ...
                    "symbol_count": len(base_context.get("file_analysis", {}).get("symbols", [])),
                    "complexity": base_context.get("file_analysis", {}).get("complexity", {})
                },
                ...
                "relevant_context": await self._get_intent_specific_context(base_context, intent),
                
                # Project awareness (summarized)
                "project_summary": self._summarize_project_context(base_context.get("project_context", {})),
                
                # Completion hints
                "completion_hints": await self._generate_completion_hints(file_path, cursor_position, intent)
            }
            
            logger.info(f"Built completion context for {intent} with {len(completion_context)} fields")
    ...
    def _create_minimal_context(self, file_path: str, error_message: str) -> Dict[str, Any]:
    ...
    async def _get_symbol_at_position(self, file_path: str, cursor_position: int) -> Optional[Symbol]:
    ...
    async def _get_related_context(self, file_path: str, current_symbol: Optional[Symbol]) -> Dict[str, Any]:
        """Get related symbols and dependencies"""
        return {
            "symbols": [],
            "dependencies": [],
    ...
    async def _get_surrounding_context(self, file_path: str, cursor_position: int) -> Dict[str, Any]:
    ...
    async def _build_suggestion_context(self, file_path: str, current_symbol: Optional[Symbol]) -> Dict[str, Any]:
    ...
    async def _generate_symbol_suggestions(self, symbol: Symbol, references: List) -> List[str]:
    ...
    async def _get_intent_specific_context(self, base_context: Dict[str, Any], intent: str) -> Dict[str, Any]:
        """Get context optimized for specific intent"""
        if intent == "suggest":
            return {
                "focus": "code_completion",
                "relevant_symbols": base_context.get("file_analysis", {}).get("symbols", [])[:5],
                "patterns": []
            }
        elif intent == "explain":
            return {
    ...
    def _summarize_project_context(self, project_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _generate_completion_hints(self, file_path: str, cursor_position: int, intent: str) -> List[str]:
        ...
        if intent == "suggest":
            if language == "python":
                hints.extend(["Consider type hints", "Follow PEP 8 style", "Add docstrings"])
            elif language == "javascript":
                hints.extend(["Use const/let", "Consider async/await", "Add JSDoc comments"])
