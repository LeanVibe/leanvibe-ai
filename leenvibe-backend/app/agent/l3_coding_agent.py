...
You have access to file analysis, file operations, and confidence assessment tools."""
    ...
    async def _analyze_file_tool(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            result = await self.ai_service._analyze_file(file_path, self.dependencies.client_id)
            ...
            if result["status"] == "success":
                ...
                self.state.update_project_context(f"analysis_{file_path}", result["data"])
    ...
    async def _file_operations_tool(
        ...
        try:
            if operation == "list":
                result = await self.ai_service._list_files(path, self.dependencies.client_id)
            elif operation == "read":
                result = await self.ai_service._read_file(path, self.dependencies.client_id)
            elif operation == "current_dir":
                result = await self.ai_service._get_current_directory("", self.dependencies.client_id)
            else:
    ...
    async def _assess_confidence_tool(
        ...
        try:
            ...
            self.state.add_conversation_entry(
                "system",
                f"Confidence assessment for: {action}",
                confidence
            )
    ...
    async def _process_user_input(self, user_input: str) -> str:
        ...
        try:
            ...
            elif "list" in user_lower and ("file" in user_lower or "directory" in user_lower):
...
"""
    ...
    def _calculate_interaction_confidence(self, user_input: str, response: str) -> float:
        ...
        elif any(word in input_lower for word in ["architecture", "design", "structure"]):
    ...
    async def reflect(self) -> Dict[str, Any]:
        ...
        try:
            conversation_summary = f"Recent conversation length: {len(self.state.conversation_history)}"
            avg_confidence = self.state.get_average_confidence()
