...
class EnhancedL3CodingAgent(L3CodingAgent):
    ...
    async def _ensure_project_indexed(self, force_refresh: bool = False) -> bool:
        ...
        try:
            ...
            if (
                not force_refresh
                and self.project_index
                and current_time - self.last_index_update < self.index_timeout
            ):
                return True
            ...
            logger.info(
                f"Project indexed: {self.project_index.supported_files} files, "
    ...
    async def _update_project_context(self):
        ...
        try:
            ...
            for symbol in self.project_index.symbols.values():
                if (
                    symbol.symbol_type in [SymbolType.CLASS, SymbolType.FUNCTION]
                    and symbol.name in ["main", "App", "Application"]
                    or symbol.name.endswith("Service")
                    or symbol.name.endswith("Manager")
                ):
    ...
    async def _create_contextual_prompt(self, user_input: str) -> str:
        """Create a contextual prompt for the LLM using AST and project context"""
        context = ""
        if self.project_context:
            context += f"Project: {self.project_context.workspace_path}\n"
            if self.project_context.architecture_summary:
                context += f"Architecture: {self.project_context.architecture_summary}\n"
            if self.project_context.key_components:
                context += f"Key Components: {', '.join(self.project_context.key_components[:5])}\n"
            if self.project_context.technology_stack:
                context += f"Tech Stack: {', '.join(self.project_context.technology_stack[:5])}\n"
        prompt = f"{context}\nUser: {user_input}\nAI:"
        return prompt
    ...
    async def _process_user_input(self, user_input: str) -> str:
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["analyze project", "project overview", "architecture"]
            ):
            ...
            # Symbol exploration
                ...
                keyword in user_lower
                for keyword in ["find symbol", "search symbol", "explore"]
            ):
                # Extract symbol name (simple heuristic)
                words = user_input.split()
                symbol_name = None
                for i, word in enumerate(words):
                    if word.lower() in ["symbol", "function", "class"] and i + 1 < len(
                        words
                    ):
            ...
            # Diagram types inquiry
                ...
                keyword in user_lower
                for keyword in ["diagram types", "available diagrams", "what diagrams"]
            ):
            ...
            # Reference finding
                ...
                for symbol in (
                    self.project_index.symbols.values() if self.project_index else []
                ):
            ...
            # Complexity analysis
                ...
                keyword in user_lower
                for keyword in ["complexity", "complex", "refactor"]
            ):
            ...
            # Architecture analysis
                ...
                keyword in user_lower
                for keyword in ["architecture", "pattern", "design"]
            ):
            ...
            # Circular dependencies
                ...
                keyword in user_lower
                for keyword in ["circular", "cycle", "circular deps"]
            ):
            ...
            # Coupling analysis
                ...
                keyword in user_lower
                for keyword in ["coupling", "dependencies", "depend"]
            ):
            ...
            # Hotspots analysis
                ...
                keyword in user_lower
                for keyword in ["hotspots", "critical", "important"]
            ):
            ...
            # Real-time monitoring commands
                ...
                keyword in user_lower
                for keyword in ["start monitoring", "monitor", "watch files"]
            ):
                ...
                keyword in user_lower
                for keyword in ["stop monitoring", "stop watching"]
            ):
                ...
                keyword in user_lower
                for keyword in ["monitoring status", "watch status", "recent changes"]
            ):
            ...
            # Indexer metrics and management
                ...
                keyword in user_lower
                for keyword in ["indexer metrics", "indexer performance", "cache stats"]
            ):
                ...
                keyword in user_lower
                for keyword in ["refresh index", "refresh project", "reindex project"]
            ):
            ...
            # Graph visualization and diagrams
                ...
                if "dependency" in user_lower or "deps" in user_lower:
                    ...
                    result = await self._generate_diagram_tool(
                        DiagramType.CIRCULAR_DEPENDENCIES
                    )
                elif "coupling" in user_lower:
                    result = await self._generate_diagram_tool(
                        DiagramType.COUPLING_ANALYSIS
                    )
            ...
            elif "analyze" in user_lower and (
...
Technology Stack: {', '.join(self.project_context.technology_stack[:10]) if self.project_context else 'Unknown'}"""
            ...
            confidence = min(
                ...
                0.5
                + (
                    analysis_data["supported_files"]
                    / max(analysis_data["total_files"], 1)
                )
                * 0.4,
            )
    ...
    async def _explore_symbols_tool(self, symbol_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate summary
            summary_parts = [
                f"Found {len(matching_symbols)} symbols matching '{symbol_name}':"
            ]
    ...
    async def _find_references_tool(self, symbol_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            references = await project_indexer.find_references(
                self.project_index, symbol_name
            )
    ...
    async def _analyze_file_with_context(self, file_path: str) -> str:
        ...
        try:
            ...
            if self.project_index:
                ...
                if related_files:
                    context_info.append(
                        f"Related files: {', '.join(set(related_files[:5]))}"
                    )
                ...
                if file_symbols:
                    ...
                    for symbol in file_symbols:
                        symbol_type = symbol.symbol_type
                        symbol_summary[symbol_type] = (
                            symbol_summary.get(symbol_type, 0) + 1
                        )

                    context_info.append(
                        f"Contains: {', '.join(f'{count} {type}s' for type, count in symbol_summary.items())}"
                    )
...
💡 Use this diagram to understand your codebase structure and identify improvement opportunities!"""
        ...
        base_summary.update(
            {
                ...
                "project_files": (
                    self.project_index.supported_files if self.project_index else 0
                ),
                "project_symbols": (
                    len(self.project_index.symbols) if self.project_index else 0
                ),
                ...
                "monitoring_active": self.dependencies.session_data.get(
    ...
    async def _start_monitoring_tool(
...
• 'start monitoring' - Enable real-time updates"""
    ...
    async def _refresh_project_index_tool(
        self, force_full: bool = True
    ) -> Dict[str, Any]:
...
✅ Project is now up-to-date and ready for analysis!"""
    ...
    async def _trigger_cache_warming_tool(
...
• Best for resource-constrained environments"""
    ...
    async def _get_file_context_tool(
        self, file_path: str, cursor_position: int = 0
    ) -> Dict[str, Any]:
        """Get comprehensive AST context for a file"""
        try:
            logger.info(
                f"Getting file context for {file_path} at position {cursor_position}"
            )

            # Use AST context provider to get rich context
            context = await self.ast_context_provider.get_file_context(
            ...
            if context.get("error"):
                summary = (
                    f"❌ Could not analyze {Path(file_path).name}: {context['error']}"
                )
            else:
...
💡 This context provides comprehensive code understanding for intelligent suggestions."""
    ...
    async def _get_completion_context_tool(
        self, file_path: str, cursor_position: int, intent: str = "suggest"
    ) -> Dict[str, Any]:
        """Get optimized context for code completion"""
        try:
            logger.info(
                f"Getting completion context for {intent} at {file_path}:{cursor_position}"
            )
...
💡 Hints:"""
    ...
    async def _suggest_code_completion_tool(
        self, file_path: str, cursor_position: int, intent: str = "suggest"
    ) -> Dict[str, Any]:
        """Generate intelligent code completion suggestions using AST context"""
        try:
            logger.info(
                f"Generating code completion for {intent} at {file_path}:{cursor_position}"
            )

            # Get completion context first
            context_result = await self._get_completion_context_tool(
                file_path, cursor_position, intent
            )
...
📝 Suggestions:"""
    ...
    async def _generate_contextual_suggestions(
        ...
        try:
            ...
            if intent == "suggest":
                suggestions.extend(
                    await self._generate_code_suggestions(language, current_file, hints)
                )
            elif intent == "explain":
                suggestions.extend(
                    await self._generate_explanation_suggestions(language, current_file)
                )
            elif intent == "refactor":
                suggestions.extend(
                    await self._generate_refactoring_suggestions(language, current_file)
                )
            elif intent == "debug":
                suggestions.extend(
                    await self._generate_debugging_suggestions(language, current_file)
                )
            elif intent == "optimize":
                suggestions.extend(
                    await self._generate_optimization_suggestions(
                        language, current_file
                    )
                )
            else:
                suggestions.extend(
                    await self._generate_general_suggestions(language, current_file)
                )
    ...
    async def _generate_code_suggestions(
        self, language: str, file_info: Dict[str, Any], hints: List[str]
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_explanation_suggestions(
        self, language: str, file_info: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_debugging_suggestions(
        self, language: str, file_info: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
