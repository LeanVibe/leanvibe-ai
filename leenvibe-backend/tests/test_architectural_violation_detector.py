...
def test_architectural_violation_detector_imports():
    ...
    try:
        ...
        assert isinstance(architectural_violation_detector, ArchitecturalViolationDetector)
...
def test_violation_type_enums():
    ...
    try:
        ...
        assert ViolationType.NAMING_CONVENTION_VIOLATION == "naming_convention_violation"
...
def test_architectural_rule_creation():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalRule, ViolationType, ViolationSeverity, ArchitecturalLayer
        )
...
def test_violation_instance_creation():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ViolationInstance, ViolationType, ViolationSeverity
        )
...
def test_architectural_report_creation():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalReport, ViolationInstance, ComponentMetrics,
...
async def test_detector_initialization():
    ...
    try:
        from app.services.architectural_violation_detector import ArchitecturalViolationDetector
...
async def test_layer_determination():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalViolationDetector, ArchitecturalLayer
        )
        
        detector = ArchitecturalViolationDetector()
        
        # Test presentation layer detection
        assert detector._determine_layer("/app/ui/components/button.py") == ArchitecturalLayer.PRESENTATION
        assert detector._determine_layer("/src/views/user_view.py") == ArchitecturalLayer.PRESENTATION
        assert detector._determine_layer("/frontend/controllers/auth.py") == ArchitecturalLayer.PRESENTATION
        
        # Test business layer detection
        assert detector._determine_layer("/app/services/user_service.py") == ArchitecturalLayer.BUSINESS
        assert detector._determine_layer("/src/business/logic.py") == ArchitecturalLayer.BUSINESS
        assert detector._determine_layer("/core/domain/entities.py") == ArchitecturalLayer.BUSINESS
        
        # Test data layer detection
        assert detector._determine_layer("/app/data/repository.py") == ArchitecturalLayer.DATA
        assert detector._determine_layer("/src/database/models.py") == ArchitecturalLayer.DATA
        assert detector._determine_layer("/storage/migrations/001.py") == ArchitecturalLayer.DATA
        
        # Test infrastructure layer detection
        assert detector._determine_layer("/app/infrastructure/config.py") == ArchitecturalLayer.INFRASTRUCTURE
        assert detector._determine_layer("/src/utils/helpers.py") == ArchitecturalLayer.INFRASTRUCTURE
        assert detector._determine_layer("/external/clients/api.py") == ArchitecturalLayer.INFRASTRUCTURE
        
        # Test test layer detection
        assert detector._determine_layer("/tests/unit/test_user.py") == ArchitecturalLayer.TEST
        assert detector._determine_layer("/src/user_test.py") == ArchitecturalLayer.TEST
        
        # Test unknown layer
        assert detector._determine_layer("/random/path/file.py") == ArchitecturalLayer.UNKNOWN
...
async def test_analyze_file():
    ...
    try:
        from app.services.architectural_violation_detector import ArchitecturalViolationDetector
        from app.models.ast_models import FileAnalysis, Symbol, SymbolType, LanguageType, Dependency
        ...
        # Mock ast_service
        with patch('app.services.architectural_violation_detector.ast_service') as mock_ast:
...
async def test_size_violation_detection():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalViolationDetector, ViolationType
        )
        ...
        component_metrics = await detector._calculate_component_metrics("/test/file.py", file_analysis)
        ...
        function_violations = detector._check_size_violations(function_rule, "/test/file.py", file_analysis, component_metrics)
        class_violations = detector._check_size_violations(class_rule, "/test/file.py", file_analysis, component_metrics)
...
async def test_complexity_violation_detection():
    ...
    try:
        ...
        violations = detector._check_complexity_violations(rule, "/test/file.py", file_analysis)
...
async def test_coupling_violation_detection():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalViolationDetector, ViolationType, ComponentMetrics
        )
        ...
        violations = detector._check_coupling_violations(rule, "/test/file.py", high_coupling_metrics)
...
async def test_layer_violation_detection():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalViolationDetector, ViolationType, ArchitecturalLayer
        )
        from app.models.ast_models import FileAnalysis, Dependency, LanguageType
        ...
        violations = await detector._check_layer_violations(rule, "/app/ui/component.py", file_analysis)
        
        # Should detect violation
        assert len(violations) >= 1
        layer_violations = [v for v in violations if v.violation_type == ViolationType.LAYER_VIOLATION]
...
async def test_anti_pattern_detection():
    ...
    try:
        ...
        violations = detector._check_anti_patterns(rule, "/test/file.py", file_analysis, component_metrics)
...
async def test_project_analysis():
    ...
    try:
        ...
        # Mock graph service
        with patch('app.services.architectural_violation_detector.graph_service') as mock_graph:
...
def test_rule_management():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalViolationDetector, ArchitecturalRule, ViolationType, ViolationSeverity
        )
...
def test_violation_filtering():
    ...
    try:
        from app.services.architectural_violation_detector import (
            ArchitecturalViolationDetector, ViolationInstance, ViolationType, ViolationSeverity
        )
        ...
        assert all(v.violation_type == ViolationType.SIZE_VIOLATION for v in size_violations)
...
if __name__ == "__main__":
    ...
    # Sync tests
    sync_tests = [
        ("Architectural Violation Detector Imports", test_architectural_violation_detector_imports),
