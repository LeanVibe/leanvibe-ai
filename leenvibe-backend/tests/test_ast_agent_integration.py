...
""")
        ...
        try:
            # Mock AST service
            with patch('app.agent.ast_context_provider.ast_service') as mock_ast_service:
    ...
    async def test_symbol_context_provision(self):
        ...
        with patch('app.agent.ast_context_provider.ast_service') as mock_ast_service, \
            ...
            mock_indexer.find_references_by_symbol_id = AsyncMock(return_value=mock_references)
    ...
    async def test_completion_context_provision(self):
        ...
        try:
            ...
            with patch('app.agent.ast_context_provider.ast_service') as mock_ast_service:
                ...
                provider = ASTContextProvider()
                context = await provider.get_completion_context(test_file, 10, "suggest")
    ...
    async def test_agent_context_understanding(self):
        ...
        with patch('app.agent.enhanced_l3_agent.ast_service') as mock_ast_service, \
            ...
            deps = AgentDependencies(
                workspace_path=".",
                client_id="test-ast-integration",
                session_data={}
            )
    ...
    def test_context_provider_error_handling(self):
        ...
        minimal_context = provider._create_minimal_context("/fake/file.py", "Test error")
...
line 7"""
        ...
        try:
            ...
            context = await provider._get_surrounding_context(test_file, cursor_position)
