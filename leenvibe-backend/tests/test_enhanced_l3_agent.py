...
''')
            ...
            data = result["data"]
            assert data["matching_symbols"] >= 3  # helper_function, another_helper, HelperClass
...
''')
            ...
            for user_input, expected_content in test_cases:
                ...
                assert any(keyword in response.lower() for keyword in expected_content.split())
...
def test_enhanced_agent_integration():
    ...
    expected_tools = [
        "analyze_project", "find_references", "impact_analysis",
        "suggest_refactoring", "explore_symbols", "check_complexity"
    ]
