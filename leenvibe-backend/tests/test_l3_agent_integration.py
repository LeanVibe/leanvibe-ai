...
class TestL3AgentIntegration:
    ...
    async def test_agent_confidence_system(self):
        ...
        response2 = await agent.run("Design a distributed microservices architecture for my application")
    ...
    async def test_agent_state_management(self):
        ...
        assert updated_state["conversation_length"] == 4  # 2 user + 2 assistant messages
...
def test_agent_state():
    ...
    assert abs(state.get_average_confidence() - 0.85) < 0.01  # Allow small floating point differences
