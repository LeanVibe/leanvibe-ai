...
def test_project_boundary_creation():
    ...
    try:
        from app.services.cascading_impact_analyzer import ProjectBoundary, ProjectBoundaryType
...
def test_cross_project_dependency_creation():
    ...
    try:
        from app.services.cascading_impact_analyzer import (
            CrossProjectDependency, ImpactPropagation
        )
...
def test_cascading_impact_structure():
    ...
    try:
        from app.services.cascading_impact_analyzer import CascadingImpact, ChangeCompatibility
        ...
        assert impact.compatibility_assessment == ChangeCompatibility.POTENTIALLY_BREAKING
...
async def test_register_project_boundary():
    ...
    try:
        from app.services.cascading_impact_analyzer import (
            CascadingImpactAnalyzer, ProjectBoundaryType
        )
...
async def test_cascading_impact_basic():
    ...
    try:
        from app.services.cascading_impact_analyzer import CascadingImpactAnalyzer, ChangeCompatibility
        
        analyzer = CascadingImpactAnalyzer()
        
        # Mock symbol dependency tracker
        with patch('app.services.cascading_impact_analyzer.symbol_dependency_tracker') as mock_tracker:
            mock_tracker.get_symbol_dependencies = AsyncMock(return_value={
                "direct_dependents": [
                    {
...
async def test_impact_summary_generation():
    ...
    try:
        from app.services.cascading_impact_analyzer import (
            CascadingImpactAnalyzer, CascadingImpact, ChangeCompatibility
        )
...
async def test_breaking_change_detection():
    ...
    try:
        ...
        test_cases = [
            ("signature_changed", {"dependency_type": DependencyType.FUNCTION_CALL}, True),
        ...
        for change_type, dep_info, expected_breaking in test_cases:
            ...
            assert is_breaking == expected_breaking, f"Failed for {change_type} with {dep_info}"
...
async def test_compatibility_assessment():
    ...
    try:
        from app.services.cascading_impact_analyzer import (
            CascadingImpactAnalyzer, ChangeCompatibility
        )
        ...
        for change_type, breaking_changes, expected_compat in test_cases:
            ...
            assert compat == expected_compat, f"Failed for {change_type} with {len(breaking_changes)} breaking changes"
