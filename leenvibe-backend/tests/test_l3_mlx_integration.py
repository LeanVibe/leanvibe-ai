...
class TestL3AgentMLXIntegration:
    ...
    async def test_enhanced_l3_agent_has_mlx_tools(self):
        ...
        deps = AgentDependencies(
            workspace_path=".", client_id="test-mlx-tools", session_data={}
        )
        ...
        for tool in mlx_tools:
            ...
            assert hasattr(
                agent, f"_{tool}_tool"
            ), f"MLX tool method _{tool}_tool not found"
...
"""
        ...
        try:
            ...
            with (
                ...
                patch(
                    "app.agent.enhanced_l3_agent.mock_mlx_service"
                ) as mock_mlx_service,
            ):
                ...
                mock_mlx_service.generate_code_completion = AsyncMock(
                    return_value=mock_mlx_response
                )
                ...
                request = json.dumps({"file_path": test_file, "cursor_position": 50})
...
"""
        ...
        try:
            ...
            with patch(
                ...
                mock_mlx_service.generate_code_completion = AsyncMock(
                    return_value=mock_explanation_response
                )

                deps = AgentDependencies(
                    workspace_path=".", client_id="test-explain", session_data={}
                )
                ...
                request = json.dumps({"file_path": test_file, "cursor_position": 30})
    ...
    async def test_mlx_refactor_code_tool_integration(self):
        ...
        with patch("app.agent.enhanced_l3_agent.mock_mlx_service") as mock_mlx_service:
            ...
            mock_mlx_service.generate_code_completion = AsyncMock(
                return_value=mock_refactor_response
            )

            deps = AgentDependencies(
                workspace_path=".", client_id="test-refactor", session_data={}
            )
            ...
            request = json.dumps({"file_path": "/test/file.py", "cursor_position": 100})
    ...
    async def test_mlx_debug_and_optimize_tools(self):
        ...
        with patch("app.agent.enhanced_l3_agent.mock_mlx_service") as mock_mlx_service:
            ...
            deps = AgentDependencies(
                workspace_path=".", client_id="test-debug-optimize", session_data={}
            )
            ...
            mock_mlx_service.generate_code_completion = AsyncMock(
                return_value=debug_response
            )

            import json

            debug_request = json.dumps(
                {"file_path": "/test/file.swift", "cursor_position": 50}
            )
            ...
            mock_mlx_service.generate_code_completion = AsyncMock(
                return_value=optimize_response
            )

            optimize_request = json.dumps(
                {"file_path": "/test/file.swift", "cursor_position": 75}
            )
    ...
    async def test_mlx_streaming_tool(self):
        ...
        deps = AgentDependencies(
            workspace_path=".", client_id="test-streaming", session_data={}
        )
        ...
        stream_request = json.dumps(
            {"file_path": "/test/file.py", "cursor_position": 100, "intent": "suggest"}
        )
    ...
    async def test_error_handling_in_mlx_tools(self):
        ...
        deps = AgentDependencies(
            workspace_path=".", client_id="test-errors", session_data={}
        )
    ...
    def test_enhanced_agent_tool_count(self):
        ...
        deps = AgentDependencies(
            workspace_path=".", client_id="test-tool-count", session_data={}
        )
        ...
        assert (
            len(agent.tools) >= 30
        ), f"Expected at least 30 tools, got {len(agent.tools)}"
