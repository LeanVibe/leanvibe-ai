...
class FileMonitorEventHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event: FileSystemEvent):
        ...
        try:
            ...
            asyncio.create_task(
                self.monitor_service._process_change(session.session_id, change)
            )

        except Exception as e:
            logger.error(f"Error handling file system event: {e}")

    def _should_monitor_file(
        self, file_path: str, config: MonitoringConfiguration
    ) -> bool:
...
class FileMonitorService:
        ...
        try:
            ...
            if not os.path.exists(config.workspace_path):
                raise MonitoringError(
                    f"Workspace path does not exist: {config.workspace_path}"
                )
            ...
            logger.info(
                f"Monitoring started for session {session_id} on {config.workspace_path}"
            )
            return True
            ...
            raise MonitoringError(
                f"Failed to start monitoring: {str(e)}", session_id=session_id
            )
    ...
    async def _process_change(self, session_id: str, change: FileChange):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Change queue full for session {session_id}, dropping change"
                )
    ...
    async def _analyze_change(self, session_id: str, change: FileChange):
        ...
        try:
            ...
            if (
                change.file_size
                and change.file_size
                > session.configuration.max_file_size_mb * 1024 * 1024
            ):
            ...
            notification = ChangeNotification.from_change(
                change, session_id, impact_assessment
            )
    ...
    async def _analyze_impact(
        self, session_id: str, change: FileChange
        ...
        try:
            ...
            if any(
                keyword in file_path.lower()
                for keyword in ["config", "setup", "build", "deploy"]
            ):
            ...
            if assessment.risk_level == RiskLevel.HIGH:
                ...
                assessment.suggested_actions.extend(
                    ["Run relevant unit tests", "Check for breaking changes"]
                )
            else:
    ...
    async def _send_notification(
        self, session_id: str, notification: ChangeNotification
    ):
    ...
    async def _update_metrics(self, session_id: str, analysis_time_ms: float):
        ...
        try:
            ...
            if metrics.average_analysis_time_ms == 0:
                ...
                metrics.average_analysis_time_ms = (
                    metrics.average_analysis_time_ms * 0.9
            ...
            if session and session.started_at:
                ...
                if elapsed_minutes > 0:
                    metrics.changes_per_minute = (
                        session.total_changes_detected / elapsed_minutes
                    )
    ...
    async def _check_for_alerts(
        self, session_id: str, change: FileChange, impact: Optional[ImpactAssessment]
    ):
        ...
        try:
            ...
            if impact and impact.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
                ...
                logger.warning(
                    f"High-risk change alert for session {session_id}: {alert.title}"
                )

        except Exception as e:
            logger.error(f"Error checking for alerts: {e}")

    async def _trigger_incremental_index_update(
        self, session_id: str, change: FileChange
    ):
        ...
        try:
            ...
            recent_changes = [
                c
                for c in session.recent_changes[:10]
                if c.is_code_file()
                and (datetime.now() - c.timestamp).total_seconds() < 60
            ]  # Last minute

            if len(recent_changes) >= 5:  # Batch threshold
                logger.debug(
                    f"Batching {len(recent_changes)} changes for incremental index update"
                )
                ...
                if updated_index:
                    ...
                    logger.info(
                        f"Updated project index: {updated_index.supported_files} files, "
                        f"{len(updated_index.symbols)} symbols"
                    )

                    # Trigger incremental graph update
                    await self._trigger_graph_update(
                        session_id, recent_changes, updated_index
                    )
            else:
                # Single file update
                logger.debug(
                    f"Queuing single file change for incremental index: {change.file_path}"
                )
                updated_index = await incremental_indexer.update_from_file_changes(
    ...
    async def _trigger_graph_update(
        self,
        session_id: str,
            ...
            for change in changes:
                if change.is_code_file():
                    await incremental_graph_service.process_file_changes(
                        session.configuration.workspace_path, [change], updated_index
                    )
    ...
    async def _trigger_symbol_dependency_update(
        ...
        try:
            ...
            # Process changes for symbol dependency tracking
            impact_analyses = await symbol_dependency_tracker.process_file_changes(
                changes
            )
            ...
            for analysis in impact_analyses:
                if analysis.impact_score > 0.5:  # Significant impact threshold
                    logger.info(
                        f"Significant symbol impact detected: {analysis.symbol_id} "
                    ...
                    if analysis.impact_score > 1.0:
                        alert = MonitoringAlert(
                            ...
                            affected_files=(
                                [
                                    symbol_dependency_tracker.symbols[
                                        analysis.symbol_id
                                    ].file_path
                                ]
                                if analysis.symbol_id
                                in symbol_dependency_tracker.symbols
                                else []
                            ),
                            risk_factors=[f"Impact score: {analysis.impact_score:.2f}"]
                            + analysis.warnings,
                            recommended_actions=analysis.suggestions,
                        )

                        logger.warning(
                            f"Symbol dependency alert for session {session_id}: {alert.title}"
                        )

            logger.debug(
                f"Processed symbol dependency updates for {len(changes)} changes, "
