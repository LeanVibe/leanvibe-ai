...
class AIErrorHandler:
    ...
    def get_error_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_errors": len(
                [e for e in self.error_history if e["timestamp"] > time.time() - 3600]
            ),  # Last hour
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                error_handler.track_error(
                    "dependency_error",
                    error_msg,
                    {"operation": operation_name, "missing_module": str(e)},
                )
                return {
                    "status": "error",
                ...
                error_handler.track_error(
                    "file_error",
                    error_msg,
                    {"operation": operation_name, "file_path": str(e)},
                )
                return {
                    "status": "error",
                ...
                error_handler.track_error(
                    "memory_error", error_msg, {"operation": operation_name}
                )
                return {
                    "status": "error",
                ...
                error_handler.track_error(
                    "timeout_error", error_msg, {"operation": operation_name}
                )
                return {
                    "status": "error",
                ...
                error_handler.track_error(
                    "encoding_error",
                    error_msg,
                    {"operation": operation_name, "encoding": e.encoding},
                )
                return {
                    "status": "error",
...
class ServiceHealthChecker:
    ...
    async def check_mlx_health(self, mlx_service) -> Dict[str, Any]:
        ...
        try:
            ...
            self.service_status["mlx"] = {
                "status": (
                    "healthy" if health_status.get("is_initialized") else "unhealthy"
                ),
    ...
    async def run_full_health_check(
        self, mlx_service, ast_service, vector_service
    ) -> Dict[str, Any]:
...
class PerformanceMonitor:
    ...
    def record_operation(
        ...
        metric["average_duration"] = (
            metric["total_duration"] / metric["total_operations"]
        )
        metric["min_duration"] = min(metric["min_duration"], duration)
        metric["max_duration"] = max(metric["max_duration"], duration)
        metric["success_rate"] = (
            metric["successful_operations"] / metric["total_operations"]
        ) * 100
