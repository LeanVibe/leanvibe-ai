...
class TestGraphQueryService:
    ...
    async def test_refactoring_suggestions(self):
        ...
        # Test with mock project
        suggestions = await graph_query_service.suggest_refactoring_opportunities(
            "test_project"
        )
...
"""
            ...
            assert (
                arch_result["status"] == "success" or arch_result["status"] == "error"
            )
            ...
            assert (
                circular_result["status"] == "success"
                or circular_result["status"] == "error"
            )
            ...
            assert (
                coupling_result["status"] == "success"
                or coupling_result["status"] == "error"
            )
            ...
            assert (
                hotspot_result["status"] == "success"
                or hotspot_result["status"] == "error"
            )
...
"""
            ...
            for query in test_queries:
                ...
                if "Graph database not available" not in response:
                    assert (
                        ...
                        or "Graph database not available" in response
                    )
    ...
    async def test_enhanced_agent_state_with_graph_capabilities(self):
        ...
        deps = AgentDependencies(
            workspace_path=".", client_id="test-state-graph-client", session_data={}
        )
...
def test_graph_integration_basic():
    ...
    from app.models.graph_models import NodeLabel, ProjectNode
    from app.services.graph_query_service import graph_query_service
    from app.services.graph_service import graph_service

    # Test basic model creation
    project = ProjectNode(id="test", name="Test Project", workspace_path="/test")
...
if __name__ == "__main__":
    ...
    asyncio.run(
        TestEnhancedAgentGraphIntegration().test_enhanced_agent_state_with_graph_capabilities()
    )
