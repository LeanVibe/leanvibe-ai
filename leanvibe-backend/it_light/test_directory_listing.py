import os, sys, pytest
sys.path.insert(0, os.path.abspath("leanvibe-backend"))
os.environ.setdefault("LEANVIBE_DATABASE_URL", "sqlite+aiosqlite:///./leanvibe_test.db")
os.environ.setdefault("LEANVIBE_SECRET_KEY", "test_secret_key")

from uuid import uuid4
from app.api.endpoints.mvp_projects import list_project_directory
from app.services.mvp_service import mvp_service
from app.models.mvp_models import MVPProject, MVPStatus, FounderInterview

pytestmark = pytest.mark.asyncio


class DummyCred:
    credentials = "token"


class DummyAuth:
    async def verify_token(self, token):
        return {"user_id": "u"}


class DummyTenant:
    def __init__(self, id):
        self.id = id


def _seed_project(project_id, tenant_id):
    proj = MVPProject(
        id=project_id,
        tenant_id=tenant_id,
        project_name="P",
        slug="p",
        description="",
        status=MVPStatus.DEPLOYED,
        interview=FounderInterview(
            business_idea="a",
            target_market="b",
            value_proposition="c",
            problem_statement="p",
            target_audience="devs",
            core_features=["f1"],
        ),
    )
    mvp_service._projects_storage[project_id] = proj


def _ensure(project_id, p, content=b"x"):
    base = os.path.abspath(os.path.join(os.getcwd(), "generated_artifacts", project_id.hex))
    os.makedirs(os.path.dirname(os.path.join(base, p)), exist_ok=True)
    with open(os.path.join(base, p), "wb") as f:
        f.write(content)


async def test_directory_listing_root_and_nested(monkeypatch):
    project_id = uuid4()
    tenant_id = uuid4()
    _seed_project(project_id, tenant_id)
    _ensure(project_id, "a.txt")
    _ensure(project_id, "dir/b.txt")

    import app.api.endpoints.mvp_projects as mod
    monkeypatch.setattr(mod, "auth_service", DummyAuth())

    # Root listing
    root_entries = await list_project_directory(
        project_id=project_id,
        credentials=DummyCred(),
        tenant=DummyTenant(tenant_id),
        _perm=None,
        path=None,
        sort="name",
    )
    names = sorted([e["name"] for e in root_entries])
    assert names == ["a.txt", "dir"]

    # Nested listing
    nested = await list_project_directory(
        project_id=project_id,
        credentials=DummyCred(),
        tenant=DummyTenant(tenant_id),
        _perm=None,
        path="dir",
        sort="name",
    )
    assert nested[0]["name"] == "b.txt" and not nested[0]["is_dir"]
